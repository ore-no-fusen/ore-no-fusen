# CodexによるMicrosoft Store提出手順

## 方針

- 5.0.0は旧MSI・NSIS利用者をStore版へ案内する移行版として、GitHub Release資産を保持する。
- 5.1.0以降はStore MSIXだけを正式配布する。
- このPCで実機確認した本番実行ファイルを再利用し、Store提出用の未署名MSIXを作る。GitHubで同じ実行ファイルを再ビルドしない。
- 通常公開の前に、このPCで開発署名MSIXを作成し、MSIX固有動作を実機確認する。

## 提出前提

1. `docs/010_RELEASE.md`のSTEP 0とSTEP 1がすべて成功している。
2. このPCの開発署名MSIXで、対象不具合と影響範囲を実機確認している。
3. 実機確認に合格した同じ本番実行ファイルから、Store提出用の未署名MSIXを作成している。
4. `main`と`develop`の5つの版ファイルが同じ版になっている。
5. Store提出用MSIXの名前、発行者、バージョン、x64、未署名を確認している。

## このPCでの提出前MSIX確認

1. アプリを終了する。
2. `develop`または作業ブランチで`.\packaging\msix\test-msix.ps1`を実行する。
3. 作成・インストールされた開発署名MSIXを起動し、次の必須項目を確認する。

| 必須確認 | 合格条件 |
|---|---|
| 既存データ | 既存付箋と設定が更新前から引き継がれる |
| 画像描き込み保存 | 線などを保存した直後に、変更後の画像が表示される |
| 画像付き付箋の複製 | 本文と画像を含む複製が隣に開く |
| iPhone連携 | PCとiPhoneの送受信、画像表示、通知遷移が動作する |

すべて合格するまで通常公開へ進まない。不合格の場合は修正をやり直し、開発署名MSIXを再作成する。

## 通常の製品申請

提出用MSIXは、版ごとに次の固定場所へ保存する。一時フォルダやリポジトリ内には置かない。

```text
D:\Users\uck\Documents\俺の付箋-Store提出\<バージョン>\ore-no-fusen.msix
```

例：`5.1.3`は`D:\Users\uck\Documents\俺の付箋-Store提出\5.1.3\ore-no-fusen.msix`へ保存する。

通常はWindows版Codexがサインイン済みブラウザでPartner Centerを操作する。VS Code版CodexはSTEP 0〜2の開発・テスト・MSIX作成を担当し、Partner Centerを操作しない。Windows版Codexがブラウザを操作できない場合だけ、ユーザーが同じ手順を手動で行う。

Windows版Codexへ申請を依頼するときは、対象バージョン、Store用MSIXの絶対パス、STEP 0〜2の完了状況、日本語・英語のリリースノートに記載する実際の変更内容を渡す。

1. Codexは`docs/010_RELEASE.md`とこの手順書を全文確認し、STEP 3の現在地、完了項目、次の操作を報告する。
2. Partner Centerの「アプリとゲーム」から「俺の付箋」を開き、「更新の開始」で新しい製品申請を作成する。既に下書き申請がある場合は、申請番号と内容を確認して再利用する。
3. 「パッケージ」で固定保存先の`ore-no-fusen.msix`をアップロードする。アップロード前に対象パス、ファイルサイズ、対象バージョンを確認する。
4. Partner Centerの検証完了後、ファイル名が`ore-no-fusen.msix`、Versionが`X.Y.Z.0`、Architectureが`X64`、状態が`Validated`であることを確認して保存する。旧版パッケージが上位版に置き換えられる通常表示は許容する。
5. 日本語（日本）と英語のStore登録情報を開き、両方の「このバージョンの最新情報」を今回の版番号と実際の変更内容に更新する。簡潔な見出しと箇条書きにし、前バージョンの文面を残さない。
6. 価格と提供の状況、プロパティ、年齢区分、Store登録情報（説明・画像・プライバシー）、申請オプション、制限付きCapabilityの説明が「変更なし」または今回意図した内容であることを確認する。
7. 認定への最終送信は外部公開申請に当たるため、Codexは実行直前にユーザーへ、対象製品、バージョン、MSIX、送信先を示して明示承認を求める。
8. 承認後、Codexが「送信して認定を受ける」を実行する。申請番号、「更新プログラムの認定中」、公開ステータスを確認してユーザーへ報告する。
9. 認定後に公開を開始する。公開設定が「認定に合格するとすぐに公開」の場合は自動で公開される。
10. 公開後、Storeから実際にインストールまたは更新し、Store画面の［開く］から起動する。
11. バージョン、Store版ショートカット、起動設定を確認する。

Microsoftアカウントへのサインイン、MFA、CAPTCHAなど本人操作が必要な場合、Codexは画面を開いたままユーザーへ引き継ぐ。ユーザーの操作完了後、同じ申請画面から続行する。

Storeが配布用MSIXへ署名するため、提出ファイルは未署名でよい。

## 公開後確認

```powershell
winget source update
winget show --id 9N4MW0V2MVVG --source msstore
winget upgrade --id 9N4MW0V2MVVG --source msstore
```

Partner Centerで認定状態、取得数、インストール数を確認する。

## 利用状況の確認

全体の入手数、インストール数、利用状況、正常性は Microsoft Partner Center の製品分析で確認する。これは Store 側の集計であり、PC アプリの GA4 同意有無とは分離する。

同意した利用者の操作改善データは Google Analytics 4 で確認する。

| 確認対象 | 画面・イベント |
|---|---|
| アプリを利用した全体規模 | Partner Center の取得・利用状況 |
| 同意後の起動 | GA4 `app_started` |
| 付箋作成 | GA4 `note_created` |
| 新規付箋の初回保存 | GA4 `first_note_saved` |
| 付箋作成失敗 | GA4 `note_create_failed` |
| 付箋・タグの利用規模 | GA4 `usage_snapshot`（固定範囲） |
| タグ、アラーム、iPhone送受信、検索、複製、整理 | GA4 `feature_used` |

GA4では `event_category`、`app_version`、`distribution`、`creation_path`、`error_category`、個数の固定範囲、許可済みの機能名だけを分析対象とし、本文、タイトル、タグ名、正確な個数、ファイル名、保存先、検索語をカスタムディメンションへ登録しない。ユーザー単位データの保持期間は2か月に設定する。

## 禁止事項

- 未署名MSIXを一般ユーザーへ配布しない。
- 5.0.0のGitHub Release資産を移行完了前に削除・置換しない。
- 5.1.0以降のGitタグ、GitHub Release、MSI、NSIS、winget community packageを新規作成しない。
- 旧版を先にアンインストールするよう案内しない。
