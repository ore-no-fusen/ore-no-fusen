...
| 5 | URLをタップ | 編集中の付箋本文内にある `http://` / `https://` URL | 編集領域外の確認画面を表示し、「リンクを開く」をタップすると現在の画面で対象URLへ移動する。保存本文は変更しない |
...
| 34 | **1.33** | 26-08-11 | メモ一覧の付箋本文内にある `http://` / `https://` URLを、安全な外部リンクとしてタップ可能にする仕様を追加。 |
