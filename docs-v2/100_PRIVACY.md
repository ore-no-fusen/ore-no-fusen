...
プライバシーポリシー v1.14 / 2026-09-21（会員分析機能は実装中）
</p>
...
| 6 | 掲示板でユーザーが明示的に選択した画像（最大3枚） | Appwrite Storage / Discord通知 | 本文だけでは伝わりにくい画面状態を開発者が確認するため。画像バイトはVercel APIやFirestoreへ保存しない |

<Note type="info">
開発者とのやりとり掲示板は、ユーザーの既存付箋本文、既存付箋の添付画像・動画、Google Drive 内の同期ファイル、Google Drive トークンを自動送信しません。掲示板でユーザーが明示的に選択した画像だけをAppwrite Storageへ直接送信し、Discord表示用の短期URLを発行します。送信成功後の画像について、30日自動削除は今回の最小版には含まれません。PC アプリは 1 日 1 回程度、自分の `conversation_id` 宛の未読返信があるかを確認します。
</Note>
...
会員番号により、氏名やメールアドレスを登録せずに開発者とのやり取りを対応付けます。固定ID、会員番号、登録日時、ランダムな分析ID、認証用秘密のハッシュをFirestoreに保存します。PCの認証秘密はWindowsで保護します。番号だけで別PCから会員情報を取得することはできません。

会員別の利用分析は、設定の「会員別分析に協力する」と従来の「匿名の利用状況を送信する」の両方を選んだ場合だけ開始します。週ごとの機能使用回数、使用日数、最終使用日、アプリ版をPC内で集計し、翌週以降の起動時にランダムな分析IDとともにGoogle Analytics 4へ送信します。週はUTCの月曜日から日曜日までです。会員番号と利用履歴はGA4へ送らず、利用履歴はVercelやFirestoreへ保存しません。本文、タグ名、検索語、添付内容、ファイルパスは含めません。初版の分析単位はアプリ個体で、別PCや保存データ削除後の再インストールを自動で同一人物とは扱いません。

送信待ちの集計はPCに最大7日分保持します。オフにすると収集を停止し、送信待ちの利用集計を破棄して、オンライン時にサーバー集計の削除を再試行します。会員番号と問い合わせの利用は継続できます。会員別・全体の利用傾向は開発者だけが認証して確認します。詳細は[009 会員番号と機能利用分析](./009_MEMBERSHIP)を参照してください。
...
| 12 | 1.11 | 26-09-04 | 会員台帳と別途明示同意する会員別分析を追加。 |
| 13 | 1.12 | 26-09-05 | 利用履歴のVercel・Firestore日次保存を廃止し、PC内月次集計とGA4送信へ変更。 |
| 14 | 1.13 | 26-09-05 | 利用分析を月次から週次へ変更。ユーザー向け利用状況画面を追加しない方針を明記。 |
| 15 | 1.14 | 26-09-21 | 開発者とのやりとりでユーザーが明示的に選択した画像を最大3枚までAppwrite Storageへ直接送信し、Discordで表示する取り扱いを追加。既存付箋の画像は自動送信せず、30日自動削除は最小版の対象外と明記。 |
