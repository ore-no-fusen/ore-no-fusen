...
settings.json の関連キー: `shortcut_new_note` / `new_note_trigger` / `shortcut_toggle_visibility` / `shortcut_arrange` / `shortcut_quick_launcher`（すべて任意。未定義なら従来どおりの既定値で動く＝後方互換）。
...
どの色の付箋からでも 右クリック →「💎 結晶にする ▸ ❓ QAにする」→ 専用ウィンドウ `/qa-create`（レシピと同じ独立窓方式・元付箋を触らない）。**材料合成はない**（付箋1枚から叩き台。合成は使われてから検討）。画面は「短い名前＋叩き台」のみ。
