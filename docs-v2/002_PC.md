...
閲覧モードでは、付箋右下にタグバッジと操作ボタンをホバー表示する。タグが0個または1個の場合は、ごみ箱ボタンの左に「📦 タグフォルダへ整理」ボタンを表示し、現在の本文を保存してからタグ0個なら Archive フォルダ、タグ1個ならそのタグフォルダへ移動する。複数タグ付きの場合は誤整理を避けるためボタンを出さず、右クリックメニューのサブメニューで移動先タグを選ぶ。
</Note>
...
| 8 | 🏷️ タグ | タグの追加・トグル・削除モードのサブメニュー | メニューを開く瞬間に `fusen_get_all_tags` で最新タグ一覧を Rust から取得（React state の古い値を使わない）。削除モードへの切り替えは `shouldReopenMenu.current = true` でメニューを閉じた直後に同じ座標で再表示する |
...
| 11 | 📦 アーカイブ | 付箋をタグフォルダ or Archive フォルダに移動する | **移動前**にアプリ固有 frontmatter（background_color・geometry 等）を `strip_sticky_fields` で除去してから書き込む。関連画像を先にコピーし、コピー成功後に元ファイルを削除するフォールセーフな順序。タグが0個または1個なら右下ホバーの 📦 ボタンからも同じ移動を実行できる。複数タグ付きならサブメニューで移動先を選択 |
...
| 12 | 2.1 | 26-06-25 | 6.2〜6.3 に付箋右下ホバー操作を追記。タグが0個または1個の付箋ではごみ箱横に「📦 タグフォルダへ整理」ボタンを表示し、複数タグ時は右クリックメニューで移動先を選ぶ仕様を明記。 |
