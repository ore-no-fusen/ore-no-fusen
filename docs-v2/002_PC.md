...
| 用語 | What（宣言知） | 「これ何だっけ」 | `term` | （将来 `Terms/`） | タブ・タグの予約のみ |
...
サブメニュー「💎 結晶にする」は全付箋に出るが、🍳 は青付箋のみ・❓ QAにする（§11.8）は全色で表示する。
...
- 検索範囲: メインの保存先フォルダ ＋ `tags/` フォルダ以下（再帰）。`Trash/`・`Archive/`・`Recipes/`・`QA/` は対象外（整理済みの付箋も材料になる。結晶は材料にならない）
...
| 用語集 | `term` タグ付き付箋 | メイン＋`tags/`（v0.2 では作成手段がないため通常は空） |
