...
思いつき（付箋）は流れて消えるが、そこから磨き上げた知識は**結晶**として永久化する。
結晶を作るのはユーザー自身であり、AI もアプリも結晶を作らない。アプリの仕事は「捕まえやすく・磨きやすく・取り出しやすく」すること。
v0.1 では 3 つの結晶のうち**レシピ**（手順集）を実装した（本章）。

### 11.1 思想（使命・3分類・判定原則）

使命: **ぼんやりした思考を捕まえ、自分の手で磨き、結晶にすることを支援する。**

```mermaid
flowchart LR
  A["ぼんやりした思考"] --> B["捕まえる<br>（Ctrl+N・色の意味）"]
  B --> C["経験する<br>（黄→桃→青・タグ）"]
  C --> D["磨く<br>（叩き台を直す）"]
  D --> E["結晶化する<br>（レシピ/QA/用語）"]
  E --> F["蓄える<br>（Ctrl+P の棚）"]
```

<p class="mermaid-caption">図 11.1-1　思考の流れとアプリの対応。各枠の上段が人の思考の工程、下段（括弧）がその工程を支えるアプリ機能。結晶化とその後の「蓄える」を支えるのが本章の機能群</p>

#### なぜ結晶を作るのか

ユーザーは作業中に「手順を資産として残そう」とは考えない。まず目の前の問題を解くために付箋へ試行錯誤を書き、できた時点では次も同じ作業をする可能性に気づかないことが多い。

本機能の目的は、完成された手順書を最初から書かせることではない。**今回できたことを少ない操作で残し、次回は自分で2秒で見つけ、見ながら実行し、使うたびに改善できるようにすること**である。この循環が続くと、ユーザーは知識管理を意識しなくても、自分専用の手順を蓄え、いつの間にかその仕事に熟達している。

```mermaid
flowchart LR
  A["今回<br/>なんとかできた"] --> B["簡単に残す<br/>（付箋から結晶化）"]
  B --> C["次回<br/>自分で2秒で見つける"]
  C --> D["見ながら実行する"]
  D --> E["気づいた点を直す<br/>（閉じる時に改善を記録）"]
  E --> C
  E --> F["いつの間にか<br/>自分の仕事が上手くなる"]
```

<p class="mermaid-caption">図 11.1-2　結晶が育てる熟達の循環。結晶は完成品を保管する箱ではなく、再利用と改善を繰り返す自分専用の知識である</p>

#### 簡単に残し、育てるための工夫

「残す」「見つける」「使う」「改善する」の各段階で、現在のアプリは次の負担を引き受ける。

<p class="table-caption">表 11.1-2　熟達の循環と現在の支援機能</p>

| 段階 | ユーザーがすること | 現在のアプリがすること |
|:---|:---|:---|
| 捕まえる | 問題解決中の考え・試したこと・結果を普通の付箋へ書く | `Ctrl+N` で即座に付箋を出し、黄→桃→青の色で思考・課題・結果を区別できる |
| 残す | できたことが書かれた青付箋を右クリックし、「💎 結晶にする ▸ 🍳 レシピにする」を選ぶ | 同じタグの黄・桃付箋を材料候補として集め、既に書いた内容を「こんなとき・どうする・補足」の叩き台へ並べ替える。短い名前も本文1行目から入るため、こだわりがなければ確認して作るだけでよい |
| 見つける | 必要になった時に `Ctrl+P` を開き、手順タブから自分で探す | 結晶を通常付箋と分けて `Recipes/` に保管し、起動時に散らかさず棚から取り出せる。現行版の検索は名前中心であり、本文・タグを含む横断検索は将来改善とする |
| 使う | レシピを開き、手順を見ながら同じ作業を行う | 「こんなとき」と番号付き手順をコンパクトに表示し、最初の一手をすぐ確認できる |
| 改善する | 実際に使って分かったことをその場で書き直し、「↩ レシピを閉じる」で返す | 変更した節を改善履歴へ自動追記し、改善回数・最終使用日時を更新してから `Recipes/` へ戻す |

AIエージェントへの指示を例にすると、初版は「計画を作らせる → 設計書とテスト方針を書かせる → 修正案と不明点を出させる → 人が確認する → 実装・テストさせる → 設計書・テスト・実装の整合を取らせる → 実機試験を人へ依頼させる」でよい。次回使った時に「修正前に影響範囲も説明させる」と気づけば、その場で1手を加える。最初から完璧に書くことではなく、**再利用した事実が次の改善を生むこと**を成功とする。
...
結晶の2つ目 = QA（Why・理由知）。「なんでだっけ」に2秒で答える。
レシピの共通基盤（結晶表示・閉じる機構・ランチャー・`ensure_named_dir`）にそのまま乗るため、本節では**レシピとの差分だけ**を定義する。共通の振る舞い（黒 `#cfd8dc`・タグで判定・起動時に開かない・改善履歴の自動追記・不変条件）は §11.2〜11.7 に従う。
...
- ランチャー QA タブ: `QA/` 内の `qa` タグ付き付箋（行アイコン ❓）。棚から外す = `qa` タグを外し `QA/` → メインへ移動（レシピと同パターン）
- 専用ウィンドウ `qa-create` は「閉じてもアプリを終了しない窓」の例外リスト（§11 の quick_launcher / recipe-create と同じ）に登録済み

### 11.9 用語機能（v0.3）— 3結晶の完成

結晶の3つ目 = 用語（What・宣言知）。「これ何だっけ」に2秒で答える。
共通の振る舞いは §11.2〜11.8 に従い、本節は用語固有の差分だけを定義する。
...
- ランチャー用語集タブ = `Terms/` 内の `term` タグ付き付箋（行アイコン 📖）。
  棚から外す = `term` タグを外し `Terms/` → メインへ移動

#### 完成宣言（26-07-09）

これで3結晶（How=レシピ / Why=QA / What=用語）が**同一作法**で
「作れる・2秒で出せる・磨ける」状態になった。§11.1 の使命に対する骨格は完成。
以後は機能を増やさず、磨いて育てる（第1弾＝§11.10 のひな形編集）。

### 11.10 結晶のひな形の編集（v0.4）

3結晶の節構成は設定画面「結晶のひな形」でユーザーが磨ける。ページ見出しは「レシピ・QA・用語のひな形」とし、対象を明示する。
