...
settings.json の関連キー: `shortcut_new_note` / `new_note_trigger` / `shortcut_toggle_visibility` / `shortcut_arrange`（すべて任意。未定義なら従来どおりの既定値で動く＝後方互換）。
...
| 2回押しの共存 | 同じ 2回押しを使うアプリ（例: Listary）があると両方反応する。検出は原理的に不可（設定画面に注記） |
...
<p class="table-caption">表 11-1　改版履歴</p>
