...
閲覧モードでは、付箋右下にタグバッジと操作ボタンをホバー表示する。タグバッジをクリックすると、そのタグフォルダを開く。タグフォルダがまだ存在しない場合は、1階層上の `tags/` フォルダを開く。`tags/` フォルダ自体がまだ存在しない場合は、保存先フォルダを開く。タグが0個または1個の場合は、ごみ箱ボタンの左に「📦 しまう」ボタンを表示し、現在の本文を保存してからタグ0個なら「アーカイブへしまう」、タグ1個なら「「タグ名」へしまう」と案内して、そのフォルダへ移動する。複数タグ付きの場合は誤整理を避けるためボタンを出さず、右クリックメニューの「タグへしまう」からしまう先を選ぶ。
</Note>
...
| 11 | 📦 タグへしまう | 付箋をデスクトップから片付け、タグフォルダまたは Archive フォルダにしまう | **移動前**に一時的なウィンドウ状態（位置・サイズ・折りたたみ・最前面・透明度など）を `strip_sticky_fields` で除去してから書き込む。背景色 `backgroundColor` は分類に使う重要情報として保持する。関連画像を先にコピーし、コピー成功後に元ファイルを削除するフォールセーフな順序。タグが0個または1個なら右下ホバーの 📦 ボタンからも同じ移動を実行できる。複数タグ付きならサブメニューでしまう先を選択 |
