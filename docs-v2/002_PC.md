...
起動中は中央のローディング画面を表示し続け、各付箋を裏で準備する。本文読込み後に2回の描画フレームを待って準備完了とし、全件完了後にまとめて表示する。1件の異常で起動が止まり続けないよう、描画待ちは10秒で打ち切り、作成済みの付箋を表示する。通常の新規付箋作成・iPhone受信・結晶表示にはこの非表示復元を適用しない。
...
- 対象: 青付箋と**共通のタグを 1 つ以上持つ** 黄（0〜1 件選択・きっかけの材料）と桃（0〜3 件選択・どうするの材料）
...
- どうする ← 青付箋本文の2行目以降＋選択した桃の本文を番号付きリスト化（7 手順以内）
