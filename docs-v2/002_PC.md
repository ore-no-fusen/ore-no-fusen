...
| 2 | △/▽ | 付箋を折りたたむ／広げる | タイトルバーの高さへ縮小し、横幅は現在幅と 320 論理 px の小さい方にする。幅と高さは `logicalSize × scaleFactor` で物理ピクセル精度に変換し、高 DPI モニタでもズレない。展開時は折りたたみ前の幅と高さへ戻す。折りたたみ中にユーザーが手動でウィンドウを広げた場合（10 物理 px 以上）は自動展開し、そのサイズを新たな「展開サイズ」として記憶する |
