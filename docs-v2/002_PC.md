...
本文の1行目をファイル名（context）として自動採用する。800ms のデバウンスで連続入力に追従。新規付箋の先頭へクリップボード画像を貼り付けた場合は、画像Markdownの直後へ改行を自動挿入してカーソルを次行へ置く。この場合は先頭の画像行を除外し、次の空でないテキスト行をファイル名として採用する。
