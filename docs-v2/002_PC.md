...
<table class="module-table">
  ...
  <tr><td>7</td><td><code>settings_theme</code></td><td>string</td><td>設定画面だけの表示テーマ。<code>light</code> / <code>dark</code> / <code>system</code>。旧設定・未知値は <code>system</code></td></tr>
  <tr><td>8</td><td><code>pc_id</code></td><td>string?</td><td>この PC を一意に識別する UUID。Drive 上の <code>pc_devices.json</code> と紐づく。初回起動時に Rust が自動生成し、ユーザーには見せない</td></tr>
  <tr><td>9</td><td><code>analytics_consent</code></td><td>string?</td><td>匿名利用状況の送信同意。未選択、<code>granted</code>、<code>denied</code> のいずれか</td></tr>
</table>
</div>
...
本文アウトラインの基本は、通常の非空行の行頭半角スペース2個を1階層として導出する自然な階層とする。箇条書き記号の入力は必須とせず、既存本文へ記号や識別子を追加しない。通常行の直後に箇条書きが連続する場合は、その通常行を親、連続する箇条書きを子として扱い、次の通常行で範囲を終了する。対象は直接入力した `・` と、表示時に `・` となるMarkdownの `-` / `*` / `+` / 番号付きリストの両方とする。箇条書き内の行頭2スペース階層も併用できる。この基本動作に加え、既存のCodeMirror Markdownパーサーが認識したATX見出し（`#`〜`######`）、箇条書き、番号付きリスト、チェックボックス付きリストの構造も階層として扱う。Markdownのlazy continuationが次の「通常行＋箇条書き」を直前のリスト項目へ含めても、画面上の通常行を独立した親として優先する。新しいライブラリや本文変換は行わず、通常行とMarkdown構造の判定を共通のアウトライン解析へ集約する。

見出しの配下は、次の同レベルまたは上位レベルの見出しの直前までとする。見出し節に含まれる下位見出し、通常文、リスト、表、画像、コードブロックは、親見出しを閉じたときにすべて隠す。Markdownリストは`ListItem`の構文範囲から親子関係を導出し、`-`、`*`、`+`および番号付きリストを記号別の独自判定へ分岐させない。子孫を持たない見出し・リスト項目には開閉UIを表示しない。Markdown構造に該当しない通常行は従来どおり2スペース階層を使用する。通常行の空行はアウトラインの区切りとし、Markdown表、画像だけの行、コードフェンスとその内部は単独では親項目にしない。

表示モードではATX見出し`#`〜`######`の記号と直後の空白を表示せず、6段階すべてを太字かつ段階的なフォントサイズで描画する。編集モードの本文と保存データは変更しない。レシピ表示の`#`・`##`に対する既存の色分けは維持する。

編集モードの `Tab` は、選択がなければカーソル位置へ半角スペース2個を入力し、複数行を選択していれば選択した各行の行頭へ半角スペース2個を追加する。行頭の半角スペース2個を通常行の1階層、4個を2階層として解釈し、Markdownとして見出しまたはリストが成立した場合は構文上の範囲を優先する。`Shift+Tab` は選択した各行（選択がなければ現在行）の行頭スペースを最大2個戻す。いずれも選択本文を削除しない。行左端の狭い操作領域をドラッグすると、共通解析が返した対象項目と全子孫を一体で移動する。通常行は移動先に合わせて2スペース単位で階層を調整するが、Markdown見出し・リストでは見出し記号、番号、インデントを自動改変せず、構文上のまとまりをそのまま移動する。

編集モードでは開閉UIを表示せず、閉じた親の子孫も含めて全行を表示する。入力した半角スペースをそのまま描画し、階層を示す追加インデントや装飾を行わない。行左端のドラッグ領域は通常時に隠し、行へホバーしたときだけ控えめに表示する。

表示モードでは、子を持つ項目だけに開閉UIを出す。閉じた親の右向き記号 `›` は常時表示し、開いている親の下向き記号 `⌄` は通常隠して行へのホバー時だけ表示する。フォーカス時はどちらも強調する。開閉記号は各項目の文字のすぐ左に19px・中太で表示し、深い項目では保存本文の行頭スペースと一緒に右へ移動する。配置幅は共通12px余白内で相殺し、記号だけのはみ出しを許可して本文位置を動かさない。編集モードも記号を出さず同じ本文開始位置を使う。階層部分は独自のピクセル幅へ置換せず、保存本文の行頭半角スペースを同じフォントと文字サイズでそのまま描画する。閉じた親は文末へ控えめな `…` を表示し、深さにかかわらず共通解析が返した全子孫を隠す。親自身と範囲外の後続項目は表示する。項目名変更、階層変更、並べ替え、保存、再起動によって開閉状態を勝手に変更しない。

長い付箋をスクロールした状態で開閉しても、操作した親行の画面上のY位置を維持する。開閉前後の行位置差だけ本文スクロールを補正し、文末でスクロール可能量が不足する場合は不足分の一時的な下余白を追加する。一時余白は次の開閉または通常のスクロール操作で解除し、本文・保存データ・付箋ウィンドウサイズには反映しない。
...
| 4 | 📋 複製 | 現在の付箋を複製する | 本文・参照画像に加え、幅・高さ、色、タグ、透明度、文字サイズ、最前面、付箋全体と本文階層の折りたたみ状態、アラーム、将来追加されたfrontmatter項目を保持する。ファイル名、識別番号、作成・更新日と、重なりを避けるX・Y位置だけを新しくし、元付箋と同じ大きさで隣に並べる |
...
| 9 | ⏰ アラームを設定 | 付箋にアラームを設定する | 「5分後」〜「1週間後」のクイックボタン＋日時直接指定の 2 モード。時刻は JST (+09:00) でフロントマター `alarm_at` に保存。時刻になると付箋が点滅し、効果音が鳴る。発火中は一時的に最前面へ表示し、停止時は発火前のピン状態へ戻す。ピン状態が未読込の場合は `false` として扱い、Tauriコマンドへ常に真偽値を渡す |
...
| 11 | 📦 タグフォルダへしまう | 付箋をデスクトップから片付け、タグフォルダまたは Archive フォルダにしまう | **移動前**に折りたたみ・最前面・透明度などの一時状態を `strip_sticky_fields` で除去するが、取り出し時に元へ戻せるよう位置・大きさ `window` は保持する。背景色 `backgroundColor` と取り出し順に使う `archivedAt` も保持する。同じフォルダに同名付箋がある場合は後からしまう本文・関連画像で上書きする。関連画像を先にコピーし、コピー成功後に元ファイルを削除するフォールセーフな順序。右側縦ツールバーの慎重操作グループから実行し、複数タグ付きならサブメニューでしまう先を選択 |
...
システムトレイの **📤 しまった付箋を取り出す** から、`Archive` または `tags/<タグ名>` にしまった付箋を元の付箋置き場へ戻す。最初に最近しまった5枚と保存場所ごとの枚数を表示し、保存場所を選ぶと新しい順に全件を表示する。名前・本文の検索は保存場所をまたいで行える。

候補カードには背景色、本文の先頭3行、最初の参照画像、保存場所、しまった日を表示する。「詳細」で全文を確認でき、画面を切り替えても選択状態を保持して複数枚をまとめて取り出せる。しまった日は frontmatter の `archivedAt` を正とし、旧版でしまった付箋に値がない場合はファイル更新日時を使う。

取り出しはコピーではなく移動である。本文と参照画像を付箋置き場へ戻してから、しまったフォルダ側の本文を削除する。同名ファイルは上書きせず結果に表示し、複数枚の一部が失敗しても成功分は直ちに元の位置・大きさで開く。取り出し後の自動整列は行わない。画像名が衝突する場合は一意な名前へ変更して本文の参照も更新する。別の付箋が同じ画像を参照している間は、しまったフォルダ側の画像を削除しない。ゴミ箱と結晶は対象外とする。画面を開いている間に別の付箋をしまった場合は、アーカイブ変更イベントを受けて一覧と保存場所別件数を再取得する。
...
設定画面・使い方・管理者ツールは、ページを開いた時点で項目数と全体像が分かる番号付きカードを共通形式として使う。カード表面は「番号・名称・短い目的・現在値」に限定し、変更操作、長い説明、パス、履歴、診断結果は「詳細」を開いた場合だけ表示する。一般は7項目、データ管理は4項目、iPhone連携は4項目、ホットキーは4項目、ひな形は3項目、使い方は3項目、管理者ツールは6項目で構成する。

表示テーマは設定画面だけに適用する。利用者はライト、ダーク、Windows の設定へ追従するシステムから選択できる。既定値と旧設定・未知値の扱いはシステムとし、付箋、クイックランチャー、確認ダイアログ、iPhone版の配色は変更しない。システム選択中は、設定画面を開いている間のWindowsテーマ変更へ追従する。
...
| No | タブ | 設定項目 | 設計意図・工夫 |
|:---|:---|:---|:---|
| 1 | 一般設定 | 言語（日本語 / English）・自動起動 ON/OFF・効果音 ON/OFF・表示テーマ（ライト / ダーク / システム）・フォントサイズ（10〜32px） | 項目が1つだけの「外観」タブは設けず、一般の「表示」グループへ統合する。表示テーマは設定画面だけに即時反映し、他画面の配色は変えない。言語・自動起動・効果音と文字サイズは従来どおり即時反映する |
...
ユーザーが連絡先を入力しなくても、設定画面内で開発者と 1 対 1 の掲示板としてやりとりできる補助機能です。ユーザーの PC に匿名の `conversation_id` を保存し、必須本文と、ユーザーが明示的に選んだ画像を最大3枚までVercel API経由でDiscord通知へ添付します。画像本体はPCからAppwrite Storageへ直接送信し、Vercel APIは画像バイトを中継しません。開発者がその通知へ返信すると、PC アプリは JST 4:00 頃に 1 日 1 回だけ自分宛の未読返信を確認し、ローカルに新着状態を保存します。右クリックメニューは通信せず、そのローカル状態だけを使って新着表示します。ユーザーと開発者の会話 UI の詳細は <a href="./007_COMMUNICATION">007 コミュニケーション設計</a> を参照してください。

<Note type="info">
<strong>怖くない設計:</strong>
PC アプリは全会話一覧を取得しません。`conversation_id` とローカルに保存した確認用トークンで、自分の掲示板だけを問い合わせます。ユーザーの既存付箋本文、既存付箋の添付画像・動画、Google Drive 内の同期ファイル、Google Drive トークンは送信しません。掲示板でユーザーが明示的に選んだ画像だけを送信します。
</Note>
...
| 2 | フィードバック送信成功時 | 設定画面の掲示板には自分の投稿本文を表示し、Discordには本文と明示添付画像を表示する | 送信済み画像を会話ログへ再表示しない最小構成で、開発者は画像をその場で確認できる |
...
| No | 分類 | 内容 |
|:---|:---|:---|
| 1 | Vercel APIへ送る | 掲示板本文、`conversation_id`、確認用トークン、アプリバージョン、アップロード済み画像の`file_id`、掲示板確認に必要な最小限の時刻 |
| 2 | Appwrite Storageへ直接送る | ユーザーが掲示板で明示的に選んだPNG・JPEG・WebP画像を最大3枚。各5MiB以下、長辺2048px以内へ必要に応じて圧縮する |
| 3 | 送らない | ユーザーの既存付箋本文、既存付箋の添付画像・動画、Vault フォルダのパス、Google Drive 内の同期ファイル、Google Drive トークン |
...
| 104 | **2.92** | 26-08-30 | §4.2.1を更新。通常行の2スペース階層を基本として維持し、CodeMirror Markdownパーサーが認識するATX見出し、箇条書き、番号付き・チェックボックス付きリストの構文範囲を折りたたみ対象へ追加した。 |
| 105 | **2.93** | 26-08-30 | §4.2.1を更新。長い付箋で本文を開閉した際、操作した親行の画面位置をスクロール補正と文末用の一時余白で維持し、カーソルと対象行が離れない仕様を追加した。 |
| 106 | **2.94** | 26-08-31 | §4.2.1を更新。通常表示モードでATX見出し`#`〜`######`の記号を隠し、6段階を太字と段階的なフォントサイズで描画する仕様へ統一した。 |
| 107 | **2.95** | 26-09-01 | 表6.3-1を更新。付箋複製で元の実ウィンドウ幅・高さと全frontmatter設定（アラームを含む）を保持し、識別情報・日付・隣接位置だけを新しくする仕様へ統一した。 |
| 108 | **2.96** | 26-09-01 | §4.2.1を更新。通常行に続く連続した`・`行を子として折りたためる階層へ追加し、複数行選択時のTab／Shift+Tabが全選択行を2スペース単位で字下げ・字上げする仕様を追加した。 |
| 109 | **2.97** | 26-09-01 | §4.2.1・表6.3-1を更新。通常行に続くMarkdown箇条書きも通常行の子として扱い、lazy continuationによる親の1行ずれを防止。アラーム停止時の最前面復元へ必ず真偽値を渡す仕様を追加した。 |
| 110 | **2.98** | 26-09-06 | §6.3.1を追加。Archive・タグフォルダから最近しまった付箋や保存場所、名前・本文で候補を探し、背景色・先頭3行・画像・日付を確認して複数枚を安全に元へ戻す仕様を定義した。 |
| 111 | **3.00** | 26-09-10 | §3.2・§6.5を更新。設定画面だけに適用するライト／ダーク／システム追従の表示テーマを追加し、旧設定はライトとして互換読込する仕様を定義した。 |
| 112 | **3.01** | 26-09-21 | §6.6を更新。掲示板の必須本文と最大3枚の明示添付画像をPCからAppwrite Storageへ直接送り、Vercel APIの検証後にDiscordへ表示する最小版を追加した。 |
