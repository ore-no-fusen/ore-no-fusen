...
| 8 | 📱 iPhoneに送る | 付箋の内容を iPhone に送信する | 送信前に `fusen_check_pro_setup` で Google Drive への実際の接続とプッシュ設定ファイルの存在を確認（ローカル状態ではなく Drive を実際に叩く）。未設定なら `{ tab: 'iphone' }` を添えて設定画面の iPhone 連携タブへ直行 |
...
| 6 | フィードバック | 開発者へのフィードバック送信 | 送信内容は Discord Webhook 経由で開発者に届く。Webhook URL は Vercel 環境変数に持ち、バイナリには含めない |
