...
本文アウトラインの基本は、通常の非空行の行頭半角スペース2個を1階層として導出する自然な階層とする。箇条書き記号の入力は必須とせず、既存本文へ記号や識別子を追加しない。この基本動作に加え、既存のCodeMirror Markdownパーサーが認識したATX見出し（`#`〜`######`）、箇条書き、番号付きリスト、チェックボックス付きリストの構造も階層として扱う。新しいライブラリや本文変換は行わず、通常行とMarkdown構造の判定を共通のアウトライン解析へ集約する。
...
編集モードの `Tab` はカーソル位置へ半角スペース2個を入力するだけとし、Markdown構造判定とは分離する。行頭の半角スペース2個を通常行の1階層、4個を2階層として解釈し、Markdownとして見出しまたはリストが成立した場合は構文上の範囲を優先する。`Shift+Tab` は現在行の行頭スペースを最大2個戻す。行左端の狭い操作領域をドラッグすると、共通解析が返した対象項目と全子孫を一体で移動する。通常行は移動先に合わせて2スペース単位で階層を調整するが、Markdown見出し・リストでは見出し記号、番号、インデントを自動改変せず、構文上のまとまりをそのまま移動する。
