...
| タグ整列 | 散らかった付箋を並べる |
| しまった付箋を取り出す | Archiveやタグフォルダから付箋を戻す |
...
システムトレイの **しまった付箋を取り出す** を選ぶと、Archiveやタグフォルダへしまった付箋を探して元へ戻せます。

1. 最初に表示される「最近しまった付箋」から選ぶか、保存場所を選びます。
2. 背景色、本文の先頭3行、画像、しまった日を見て、戻す付箋にチェックを付けます。内容を確かめたい場合は **詳細** を選びます。
3. **○枚を取り出す** を選びます。複数の保存場所から選んだ付箋もまとめて戻せます。

名前や本文でも検索できます。同名の付箋がすでに元の場所にある場合は上書きせず、その付箋だけを結果に表示します。取り出した付箋は、しまう前の位置と大きさですぐにデスクトップへ表示されます。取り出し画面を開いたまま別の付箋をしまった場合も、一覧と枚数が自動的に更新されます。
