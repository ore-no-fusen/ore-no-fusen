...
MSI/NSIS は本気版として扱います。自由な保存先、アプリ内更新を前提にします。
...
| 5 | シンボリックリンク作成 | 不可（OS の権限制約） | 不可（OS の権限制約） |
...
ファイルのシンボリックリンク（symlink）作成は Windows の制約により管理者権限／開発者モードが必要で、MSIX・MSI のどちらでも一般ユーザーは使えません（配布形態の差ではありません）。本アプリは symlink を作りません。
...
| No | 項目 | 方針 |
|:---|:---|:---|
| 1 | シンボリックリンク | MSIX・MSI とも一般ユーザーは作成不可（OS の権限制約）。本アプリは作らない |
