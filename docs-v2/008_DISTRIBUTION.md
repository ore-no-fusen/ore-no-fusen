...
MSIX版の初回起動時は、旧デスクトップ版と共有している`auto_start`設定をOFFへ移行します。これは旧版の実行ファイルがレジストリRunキーを再登録することを防ぐためであり、MSIX版自身のStartupTask状態は変更しません。既にWindowsへ残っている旧版の項目は「スタートアップ アプリ」で一度無効化します。
...
| 4 | v2.2 | 26-08-16 | MSIX初回起動時に旧デスクトップ版の共有自動起動設定をOFFへ移行し、旧Runキーの再登録を防止する仕様を追加 |
