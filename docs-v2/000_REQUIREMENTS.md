...
<table class="module-table" style="font-size:12px; width:100%; border-collapse: collapse; margin-bottom:24px;">
  ...
  <tbody>
    ...
    <tr>
      ...
      <td style=""><strong>同意制の匿名利用状況</strong>: 初回確認でユーザーが明示的に「協力する」を選んだ場合だけ、起動、付箋作成・初回保存、復元成否、安全なエラー分類、アプリ版、配布形式、言語を Google Analytics 4 へ送信する。加えて、付箋枚数・タグ付き付箋枚数・タグ数は個数そのものではなく固定範囲で送り、タグ、アラーム、iPhone送受信、検索、複製、整理の利用有無を機能名だけで送信する。同意前および拒否後は GA4 を読み込まず、付箋本文、タイトル、タグ名、画像、ファイル名、保存先、検索語、個人情報を送信しない。拒否しても全機能を利用でき、設定画面からいつでも変更できる。Store 全体の入手数・利用状況は Partner Center で確認し、アプリ独自計測を強制しない。</td>
    </tr>
  </tbody>
</table>
...
| 12 | **v2.20** | 26-08-02 | SPEC-NF-03-04を、ノー・テレメトリーから明示同意後だけの匿名利用状況へ変更。同意前通信禁止、送信禁止項目、拒否時の全機能提供、Partner Centerとの役割分担を明記。 |
| 13 | **v2.21** | 26-08-29 | SPEC-NF-03-04へ、固定範囲による付箋・タグ利用規模と、内容を含まない主要機能の利用イベントを追加。 |
