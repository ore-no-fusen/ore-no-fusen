...
<table class="module-table" style="font-size:12px; width:100%; border-collapse: collapse; margin-bottom:24px;">
  ...
  <tbody>
    ...
    <tr>
      ...
      <td style=""><strong>同意制の週次利用状況</strong>: 「匿名の利用状況」と「会員別分析」の両方へ明示同意した場合だけ、機能ごとの週間使用回数・使用日数・最終使用日をPC内で集計し、翌週以降の起動から60秒経過後にランダムな分析IDで Google Analytics 4 へ送信する。週はUTCの月曜日から日曜日までとする。通常操作はメモリ上の加算だけとし、操作ごとのディスク書込み・Tauri通信・外部通信を禁止する。会員番号、付箋本文、タイトル、タグ名、画像、ファイル名、保存先、検索語、個人情報をGA4へ送信しない。拒否しても全機能を利用でき、設定画面からいつでも変更できる。集計の完全性より付箋の表示・入力・保存を優先する。ユーザー向け利用状況画面は追加しない。</td>
    </tr>
  </tbody>
</table>
