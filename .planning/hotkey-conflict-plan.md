...
作成: 26-07-04 ／ 状態: **全 Phase 完了（実機 UAT 待ち）**

| Phase | 状態 | コミット |
|---|---|---|
| 1 HotKeyManager 集約 | ✅ 完了 | 0cfe849, bfed95f |
| 2 判定・変更コマンド | ✅ 完了 | cb70452 |
| 3a 2回押し判定ロジック | ✅ 完了 | c67ee67 |
| 3b Win32フック＋統合 | ✅ 完了 | 7132a0e |
| 4 起動時失敗ダイアログ | ✅ 完了（pull方式） | 062683d |
| 5 設定画面UI | ✅ 完了 | d0b030e |
| 6 テスト＋設計書 | ✅ 完了（shortcutKey テスト7本＋docs-v2/002_PC.md §10 新設） | 後続コミット |

**残タスク**: 実機 UAT（ユーザー確認）— ①設定画面での競合チェック表示 ②2回押しでのポン出し ③起動時失敗ダイアログ（他アプリでキー占有して再現）
...
| 4 | 26-07-04 | Phase 1〜3b 実装完了・コミット済み（進捗表を冒頭に追加）。レビューで hotkey_apply の旧キー解除失敗を致命エラーにしない修正と、未知ショートカット発火の無視を反映 |
