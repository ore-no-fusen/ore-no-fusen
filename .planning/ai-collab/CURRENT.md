## 現在のリリース作業（26-08-10・最初に読む）

- 基準手順: `docs/010_RELEASE.md`を作業前に必ず読む。
- 現在地: STEP 3-1完了。5.1.3のStore提出用MSIXをこのPCへ準備済み。
- 提出ファイル: `D:\Users\uck\Documents\俺の付箋-Store提出\5.1.3\ore-no-fusen.msix`
- ファイル容量: 53,259,032バイト。
- 次の工程: STEP 3-2。Partner Centerの新しい製品申請へ上記MSIXをアップロードする。
- STEP 3-2以降はユーザーのPartner Center画面操作が必要。画面を確認しながら案内する。
- 禁止: 再ビルド、再ダウンロード、別フォルダへの複製、手順にないテストを行わない。
- 未コミット文書: `docs/010_RELEASE.md`と`docs/store-submission.md`。保存先、実施場所、概要図・詳細図、時間記録を更新済み。
