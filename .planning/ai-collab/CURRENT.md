...
- 専用worktree `C:\tmp\ore-no-fusen-natural-outline`、ブランチ `codex/natural-outline` で実装中。5.1.7側のworktreeは変更しない。
...
- 実機確認待ち: Store版5.1.3が起動中のため、Tauriの単一起動制御で専用worktree版が終了した。未保存データを守るため自動終了はせず、ユーザーがStore版を閉じた後に再起動する。
