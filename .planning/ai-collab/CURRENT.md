...
- 投稿画面で全会員、古参（10000〜10049）、新規（10100以降）、今週の機能利用あり／なし、登録済み会員番号1件を選択できる。個別宛て・会員番号区分はAPIでも対象外会員に本文を返さず、Rustでも一致を確認する。機能利用区分はPC内だけで判定できるため、機密本文に使わない注意を投稿画面と仕様書に記載。
- 宛先の不明値・未登録番号は保存を拒否。ブラウザから未登録番号の拒否、登録済み番号の保存、会員区分への切替を確認。対象外会員へのAPI返却がないことをテストした。
- 検証: 対象Vitest 12件、ダッシュボードNode/ブラウザ5件、Rust対象1件、全Vitest 638件、E2E 41件成功・5件skip、TypeScript、Rust check、VitePress build、構文・差分検査に成功。実際の配信は未実施。

### 2026-09-24 開発者ホットライン・実動作テストで判明した修正

- `codex/hotline-test-fixes` で、解析同意を拒否した会員のheartbeatが呼ばれない分岐と、お便り本文を未検証HTMLで挿入する表示処理を修正。解析送信は従来通り同意した場合だけとし、heartbeatは同意によらず実行する。
- お便り表示は既存の太字・HTTP(S)リンク・改行を維持し、HTMLタグと`javascript:`リンクを実行可能な要素にしない。受信ウィンドウ生成の単体テストとブラウザE2Eを追加。
- 検証: 関連単体テスト7件、全単体テスト637件、TypeScript型検査、`e2e/announcement.spec.ts` 1件成功。開発者投稿フォームの宛先指定は追加要望として継続中。
...
- 確認後にこのPCで`develop`へ統合し、GitHubへは`develop`だけを1回pushする。作業中の機能ブランチはpushせず、Pull Requestはユーザーの明示依頼時だけ作る。
- push後は変更対象に必要なCIとdevelop Previewだけを確認する。STEP 0ではMSIX CIの失敗を調査・修正しない。
...
- このPC、`origin/develop`、`origin/main`が同じ5.3.0コミット`553567a`で、通常作業場所に未コミット変更がないことを確認した。
- 画像機能ブランチへ5.3.0を取り込み、Markdown表示、横スクロール、設定画面のバージョン取得、PC設計書の競合を両機能を残す形で解消した。PC設計書はv3.01とした。
- 試作途中の引き継ぎファイルと無効化済み接続診断APIは統合対象から除外した。対象17ファイル・113テスト、TypeScript、VitePressビルド、差分検査に成功。次はPR更新、`develop`マージ、CI・Preview・PC実機確認を行う。

### 2026-09-21 やりとり画像添付の設計書を最終実装へ同期

- `docs-v2/007_COMMUNICATION.md`へ、必須本文と最大3枚の画像をPCからAppwrite Storageへ直接送り、Vercel APIでメタデータ検証後にDiscordへ表示するMermaidシーケンス図を追加した。
- 送信側会話ログは本文だけ、Discord画像返信・30日自動削除は対象外、送信失敗時は今回の画像削除を試みるという最小版の境界を明記した。
- `docs-v2/002_PC.md`の掲示板UI・送信情報と、`docs-v2/100_PRIVACY.md`の画像保存・非自動送信・保持条件を現行実装へ同期した。

### 2026-09-21 通常開発のブランチ統合ルールを固定

- 今後すべての機能追加・不具合修正・文書変更は、最新の`develop`から作る`codex/<作業名>`ブランチで行う。
- 作業ブランチはこのPC内だけで使用し、対象テスト・型検査・必要な実機確認後、このPCで`develop`へ統合する。GitHubへは`develop`だけをpushする。
- リモート機能ブランチとPull Requestは明示依頼時だけ使用する。push後は変更対象に必要なCIとdevelop Previewだけを確認する。
- `main`は正式リリース専用とし、機能ブランチから直接マージしない。正式な正本を`docs/010_RELEASE.md`へ追加した。

### 2026-09-21 Markdown本文表示のReactキー重複警告を修正

- 長い付箋の表示時に、通常行の元行番号とコードブロック／表のグループ順が同じ数値キーになると、Next.js開発版で重複キー警告が表示されていた。
- コードブロックと表のキーを種別＋元の開始行番号へ変更し、通常行の数値キーと衝突しないよう最小修正した。本文・保存・画像送信は変更していない。
- 再現テストを追加し、MarkdownRenderer 20件、TypeScript、差分検査、起動中Tauriの再コンパイルに成功。

### 2026-09-21 やりとり画像添付・実機送信確認とバージョン表示修正

- Tauri開発版から本文と画像を送信し、Discordで本文と画像が同時に表示されることをユーザー実機確認済み。完成条件を達成した。
- Discord通知のバージョンが固定値 `Unknown` だったため、送信直前にTauriの実アプリバージョンを取得して送る最小修正を行った。画像の送信・表示仕様は変更していない。
- 開発版を任意のPreview APIへ接続できる開発時限定の環境変数を追加。production/developの既定接続先は変更していない。
- 対象Vitest 18件、TypeScript、差分検査、起動中Tauriの再コンパイルに成功。作業ブランチは `codex/image-attachments-probe`、`develop` は未変更。
...
- シェルの否定条件を廃止し、ステージ済みパスを分類する `scripts/precommit-classify.mjs` と境界テスト3件を追加。
- 実際のコード3ファイルをステージした通常コミットで、pre-commitがTypeScript・全Vitest 114ファイル573件を実行して成功することを確認。修正コミット `a0402da`。

### 2026-09-06 問い合わせへの会員番号自動付与を明示

- 問い合わせ通知は、すでに認証済み会話IDと会員台帳をサーバー側で照合し、一般会員番号を自動付与する実装になっていることを確認。
- 問い合わせ画面と「開発者とのやりとり」画面へ一般会員番号を読み取り専用で表示し、「問い合わせに自動で付与されます」と明示した。固有ID・分析IDは表示も送信もしない。
- 会員番号は登録順と利用開始時期の目安として扱い、使用頻度・習熟度は同意済みの別分析結果から判断する。
- 新規表示テスト2件、TypeScript、差分検査に成功。既存の付箋編集関連の未コミット変更には触れていない。
### 2026-09-06 付箋編集の Shift+上下範囲選択停止を修正

- 独自の表示行座標計算が移動不能になっても `preventDefault` で標準動作を止めるため、複数行選択が途中で詰まる不具合を修正。
- `Shift+ArrowDown` / `Shift+ArrowUp` は CodeMirror 標準移動を優先し、行途中の列位置から短い行・折り返し境界へ来て文字位置が進まない場合だけ、次／前の論理行へ選択端を進める安全網を追加。本文・保存・書式処理は変更なし。
- 行途中開始・短い行・文頭文末の回帰テストを追加。対象13件、全Vitest 112ファイル・568件、TypeScript検査に成功。さらに実ブラウザー上のCodeMirrorへ30行を入力し、行途中から `Shift+↓` を120回送って選択本文が文末まで一致するE2E 1件に成功。Tauri開発版でもユーザー実機確認に合格。修正コミット `bc10302`。

### 2026-09-01 5.2.1候補・複製完全性と階層入力の追加修正

- 5.2.1開発署名MSIXの実機確認で、複製直前の未保存状態によりアラームと本文階層の折りたたみ状態が複製されないことを確認。リリースをSTEP 0へ戻した。
- 根本原因は、複製要求が現在の画面状態を保存せず、ディスク上の旧ファイルをRust側で複製していたこと。複製前に現在の本文・frontmatter保存完了を待ち、保存失敗時は複製要求を送らないよう修正した。
- 複数行選択時のTabが選択本文を2スペースへ置換していたため、Tabは選択全行の行頭へ2スペース追加、Shift+Tabは選択全行から最大2スペース削除へ修正。選択本文は保持する。
- 通常行の直後に連続する日本語の`・`箇条書きを、その通常行の子として折りたためる階層へ追加。次の通常行で終了し、`・`行内の2スペース階層も併用する。既存の2スペース・Markdown階層、本文、保存形式、表示UIは変更しない。
- 対象38テストとTypeScript検査に成功。全体検査と新しい開発署名MSIXでの再実機確認が必要。
- 実機で、画面上の`・`がファイル内ではMarkdownの`- `であるケースに未対応と判明。通常行に続く直接入力`・`とMarkdown箇条書きの両方を通常行の子にし、Markdown lazy continuationが次の「通常行＋箇条書き」を直前のリストへ取り込む場合も通常行を親として優先するよう修正した。
- アラーム停止時に、未読込のピン状態が`null`として`fusen_set_always_on_top`へ渡りRuntime Errorになる既存不具合を実機で確認。開始前状態の記録・停止時の復元を必ず真偽値へ正規化した。
- 追加修正後、対象37テスト、全Vitest 106ファイル・545件、TypeScript、VitePress buildに成功。設計書v2.97へ同期。開発版で再実機確認待ち。

### 2026-08-31 Microsoft Store認知改善（Submission 17・下書き）

- 直近1か月のStore分析はページ閲覧294、インストール75、転換率25.51%、インストール成功率96.15%。転換より露出不足を優先課題と判断した。
- ユーザー承認後、パッケージ・価格・製品名を変更しない登録情報専用のSubmission 17を作成した。
- 日本語・英語とも、5.2.0の核である自然な階層・折りたたみを冒頭で伝える説明、短い説明、機能一覧へ更新した。
- 検索語は「折りたたみメモ / Collapsible Notes」「アウトライン / Outliner」「ローカル保存 / Local Notes」など、主要用途と差別化点を含む7語へ更新した。
- 既存の英語4枚は実画面素材として維持。日本語は現状1枚のため、個人情報を含まない5.2.0実画面を用意できた段階で4枚へ増やす。誤認を招く合成画像は使わない。
- 下書き保存まで実施し、最終の「送信して認定を受ける」はユーザーの実行時確認前には押さない。
- Store画像作成中に、通常表示では`#`だけを見出し装飾し、折りたたみ解析が対応する`##`〜`######`は記号が残る表示差を確認した。通常表示もATX見出し6段階を太字・段階的サイズで描画するよう最小修正し、編集本文・保存データ・折りたたみ解析・レシピの既存色分けは維持した。
- 検証: MarkdownRenderer対象18件、TypeScript、VitePress buildに成功。要求仕様とPC設計書v2.94を更新した。
- Store画像撮影時に各Tauri付箋へ重複表示されるNext.js開発インジケーター（左下の黒い`N`）は使用していないため、`next.config.mjs`の`devIndicators: false`で開発版のみ非表示にした。HMR、エラー表示、DevTools、本番版の動作には影響しない。設定反映後にTauri開発版を再起動し、正常起動を確認した。

### 2026-08-29 本文アウトライン・階層折りたたみ（専用worktree）

- 全体へ展開する設計原則: 情報は常時すべて見せず、必要なときだけ自然に展開できるようにする。一覧性を高め、情報量と視覚的ノイズを減らすことで、デスクトップ領域を占有しやすい付箋アプリの弱点を緩和する。機能追加では「隠すための複雑な管理」を増やさず、普段の入力と表示に溶け込む段階的開示を優先する。
- 専用worktree `C:\tmp\ore-no-fusen-natural-outline`、ブランチ `codex/natural-outline` で実装し、ユーザー実機確認後に `develop` へ競合なしで統合した。5.1.7の匿名利用状況集計も保持している。
- 採用UI: 事前スケッチのC案。編集モードは全行を表示して階層編集に専念し、開閉UIを出さない。表示モードだけ、子を持つ行へ見える濃さの `▶` / `▼` を表示する。
- 本文は既存Markdown文字列のまま、行頭2スペースを1階層として解釈する。既存データ変換と新規ライブラリ追加は行わない。
- 開閉状態は平坦なfrontmatter `outlineCollapsed` へ保存する。既存の付箋全体の `folded` とは別管理。
- 実装済み: 階層解析、親子まとめてのドラッグ、表示モードでの開閉、名前変更・行追加・移動時の状態追従、保存復元。実機フィードバックにより、編集モードの開閉を廃止した。Tabはカーソル位置へ半角スペース2個を入れるだけ、行頭2スペースを表示時の1階層として解釈する単純な操作へ変更した。
- 実機修正: 最上位の三角が左側へはみ出す問題を解消し、閉じた親の文末へ `…` を表示する。既存のTabによるツールバーフォーカス移動を削除し、空白2個入力との競合を解消する。
- 表示UI再調整: Obsidian同様、閉じた親の `›` は常時表示、開いた親の `⌄` は行へのマウスオーバー時だけ表示する。子のない行には表示しない。
- 編集UI再調整: Tabで入力した半角スペース2個に12pxの階層装飾が重なり、改行後の空行と位置がずれる問題を修正。編集モードでは入力スペースをそのまま表示し、階層の意味付けとインデント描画は表示モードだけで行う。
- Workflowy参考の最小方針: 編集・表示の双方に開閉記号用12px余白を確保して本文開始位置を揃える。表示時も独自の階層幅へ置換せず、入力された半角スペースを同じフォント・文字サイズでそのまま描画する。ノードUI、ズーム、多数のショートカットは持ち込まない。
- 位置合わせ修正: 表示側だけ外側4pxと開閉用12pxが重なり、編集側の12pxはグローバルCSSの `!important` で4pxへ戻されていた。表示の左外側余白を0にし、`›` を可能な限り左へ配置。編集側の実効左余白を12pxにして本文開始位置を一致させる。
- 開閉記号位置: ユーザー確認後、全記号を左端一列ではなく対象文字のすぐ左へ配置。行頭スペースの後へ記号を移し、負の余白で記号幅を相殺するため、編集・表示の本文位置は維持する。
- 開閉記号サイズ: 12pxの配置幅と本文位置は維持し、右向き・下向きとも19px・中太へ拡大。記号部分だけのはみ出しを許可し、下向き記号もボタン枠の中央へ配置する。
- 開閉記号のカーソル重なり対策: 下向き記号とクリック範囲は10px、右向きは3px上へ移動する。本文位置、行間、12pxの配置幅は変更しない。
- ドラッグカーソル: 標準の手カーソルはホットスポットを調整できないため、24pxの独自SVGへ変更。開いた「つかむ手」の外観を維持し、人差し指の先端 `(11, 1)` を実際のポインター位置にする。未対応環境では標準の `grab` へフォールバックする。
- 折りたたみカーソル: 開閉記号の標準指さしカーソルを20pxの独自SVGへ変更。指先を実際のポインター位置にし、独自グラブと線・色を統一する。対象は開閉記号だけで、未対応環境では標準の `pointer` へフォールバックする。
- Markdown構造拡張: 通常行の2スペース階層を基本として維持し、既存のCodeMirror MarkdownパーサーによるATX見出し、`-` / `*` / `+`、番号付き・チェックボックス付きリストの構文範囲を追加する。判定は `app/utils/outline.ts` に一本化し、表示UI・保存形式・Tab入力は変更しない。見出し配下の表・コードも親を閉じたときは一緒に隠す。Markdown構造のドラッグは記号や見出しレベルを自動変更せず、構文上のまとまりをそのまま移動する。
- 開閉位置固定: 長い付箋の文末付近で開閉すると本文スクロール上限が変わり、押した親行がカーソルから離れる問題を修正。開閉前後の親行Y位置を測ってscrollTopを補正し、不足時だけ一時的な下余白を追加する。次の開閉・通常スクロールで余白を解除し、本文と保存データは変更しない。
...
- STEP 0-1開始: 2026-08-29 06:15:01 +09:00。今回から各工程の開始・終了・秒数を必ず記録する。
- 目的: 同意済みユーザーについて、付箋が何枚程度使われているか、タグ・アラーム・iPhone送受信・検索・複製・整理が使われているかを、内容を送らず判断できるようにする。
- 実装中: `usage_snapshot` は付箋数・タグ付き付箋数・タグ数を `0 / 1-5 / 6-10 / 11-20 / 21-50 / 51+` の固定範囲で送る。`feature_used` は許可済み機能名だけを送る。
- 禁止維持: 本文、タイトル、タグ名、正確な個数、ファイル名、保存先、検索語、個人情報は送らない。同意前・拒否後は送らない。
...
- 複製時は、ファイル名・連番・作成更新日・重複を避ける表示位置以外を元付箋からそのまま引き継ぐ。アラーム、幅・高さ、色、透明度、フォント、常に手前、折りたたみ状態、画像、将来追加されるfrontmatter項目も保持する。
- 元ウィンドウの実測外寸は複製位置の計算だけに使用する。複製先の幅・高さには再利用せず、元ファイルに保存された値を使うことでDPI端数によるサイズ差を防ぐ。
- 検証: TypeScript型検査、Rustのfrontmatter完全保持テスト、`git diff --check`に成功。残作業は開発版で元付箋と複製先の幅・高さ、アラーム設定が一致することの実機確認。
- 実機不合格: 画面上では `予約` の折りたたみとアラームが有効でも、元ファイル0063は `outlineCollapsed: [2]`・アラームなしの古い状態だった。旧実装は保存後にRustがディスクを再読込するため、複製先0065も古い状態になった。
...
## 2026-09-03 利用指標の受信・設定確認（読み取り調査）

- GA4実画面の2026-08-06〜09-02を確認。app_started 139回/10ユーザー、usage_snapshot 109回/7ユーザー、feature_used 21回/2ユーザー、note_created 20回/4ユーザー、first_note_saved 18回/4ユーザー。イベントの受信は成立している。ユーザー数はGA4上の識別数で、同意済み計測に限る。実人数や全利用者数とは断定しない。
- GA4「管理→カスタム定義」のカスタムディメンションは0件。付箋枚数範囲、タグ関連範囲、iPhone設定、機能名を通常レポートで分析する設定が未完了。登録候補は note_count_bucket / tagged_note_count_bucket / tag_count_bucket / iphone_enabled / feature_name。起動版の分析用に app_version / distribution も候補。
- コード確認: usage_snapshotは主ウィンドウ起動時の保有状況で、日々の編集・閲覧や継続利用を表すイベントではない。first_note_savedは利用者の生涯初回ではなく新規付箋の初回保存。開発版も同一GAプロパティへ送るため、開発者の検証分を除外済みとは言えない。app_versionはapp_startedだけに付くため今回の全イベントを5.2.1限定とは判定できない。
- 設定・ソースの変更は行っていない。最小の次工程はGA4側のカスタムディメンション登録。継続利用と開発版除外は別途計測仕様を決める。

## 2026-09-04 GA4カスタムディメンション登録完了

- ユーザー承認に基づき、ore-no-fusenプロパティ524376317へイベントスコープ7項目を登録し、GA4一覧の1～7/7と各パラメータを確認した。
- 付箋枚数の範囲=note_count_bucket、タグ付き付箋枚数の範囲=tagged_note_count_bucket、タグ種類数の範囲=tag_count_bucket、iPhone送信の有効設定=iphone_enabled、利用した機能=feature_name、起動時のアプリ版=app_version、起動時の配布形式=distribution。
- アプリソースは変更していない。前回の関連テスト19件成功とGA4受信実績が基本送信の根拠。全操作の無欠損・重複なしを保証する検査ではない。
- 現行の制約: usage_snapshotは起動時の保有状況のみ、first_note_savedは新規付箋単位、app_versionはapp_startedのみ、distributionはdesktop_app固定で開発版とStore版を区別しない。継続利用・一般ユーザーだけの厳密な分析には別途アプリ側の改善が必要。

## 2026-09-04 匿名個人別・全体別の機能利用分析の準備

- ユーザー目的: ①匿名A/Bごとと全体の機能使用状況を知る、②おすすめ機能と照合して改善提案を検討する。
- 準備案をFEATURE_USAGE_ANALYSIS_PLAN.mdへ保存。個体識別、機能カタログ、未観測/未計測の区別、集計母数、最新スナップショット、提案ルール、実装順、検証条件を定義。表のA/Bは架空例で実測ではない。
- 追加確認: saveNoteContentは保護による正常returnで保存をスキップできるが、呼び出し元のfirst_note_saved/alarm_setはawait完了だけで計測する。実保存結果を区別する最小修正が必要。同意ダイアログの項目説明も既存の機能利用/保有範囲に追従していない。
- アプリ実装・新規外部連携・アドバイス配信は未実施。AGENTS.mdの修正前説明・確認ルールに従い、具体化した準備案の確認後に実装へ進む。GA個体識別の窓間/再起動継続性、BigQuery利用環境は実装前検証項目。

## 2026-09-04 会員番号と機能利用分析の統合提案

- FEATURE_USAGE_ANALYSIS_PLAN.mdへ会員番号自動発行・既存会話移行・同意済み分析ID対応・リスク・修正ファイルを追記。実装は未着手。
- 実設定UIはcomponents/ui/settings-page.tsx。既存会話ID/secret_tokenはapp/utils/feedbackConversation.tsでlocalStorage管理している。
- 投稿API2件は所有確認前の履歴参照およびprovided tokenでの会話作成経路があり、会員番号拡張前の優先修正項目として記載。攻撃成立を実環境では検証していない。

## 2026-09-04 会員番号は通番へ変更

- ユーザー指定によりランダム表示番号案を通番（000001〜）へ変更。初回登録日時も記録し、登録順と時期を確認できるようにする。
- サーバーで原子的採番・登録冪等性・本番/検証分離・番号再利用禁止を設計。番号と秘密認証は分離。
- 最大番号は累計発行の目安で実人数ではない。既存ユーザーの過去の利用開始順も表さない。管理画面で登録件数と期間内利用会員件数を別表示する。
- FEATURE_USAGE_ANALYSIS_PLAN.mdを更新。アプリ実装は未着手。

### 会員番号の表示形式確定
- ユーザー指定: 1、2、3…と1から採番し、ゼロ埋め・接頭辞を付けない。過去記載の000001形式を置き換える。実装は未着手。

## 会員区分別の開始番号・最新指定

- ユーザー確定: 一般会員10000から、有料会員1から、区分別の独立通番。ゼロ埋めなし。
- FEATURE_USAGE_ANALYSIS_PLAN.mdへ反映。以前の1始まり/1000始まり指定に優先する。
- 内部IDは維持し、区分+番号で衝突を防止する。一般→有料への表示番号移行は未確定。実装は未着手。

## ソース接続確認・提案の訂正

- 設定UI components/ui/settings-page.tsx:2357/2562はconversation/messages APIへ投稿する。会話IDと秘密トークンはapp/utils/feedbackConversation.ts:43でlocalStorageに作成・保存。Rust管理の会員番号は現状存在しない。
- API messages/route.tsはDiscord Webhookへ内容・会話ID・直近履歴を送信後、store.createConversation/appendMessageで保存。Firestore設定が揃う場合はFirestore、未設定ではメモリ実装を使う。稼働環境の変数は未確認。
- Discord返信はdiscord/ingest/run.tsで許可開発者・元メッセージ対応・重複を検査して保存。設定掲示板はpollで取得しackで既読化。主画面のuseFeedbackConversationUnreadCheckは起動時と30分間隔で日次条件を確認し、JST4時以降1日1回確認を試行する。即時pushではない。
- GA4はAnalyticsLoader/analytics.tsの別経路。会話ID・決済・会員番号と結合する実装はない。
- 設定の応援リンクは外部ページを開き、app/endroll/EndrollContent.tsxはStripe Payment Linkへ誘導する。このリポジトリのapp/api、Rust、設定ストアには購入完了を受けてアプリ個体を有料と認定する接続実装を確認できなかった。Stripe外部設定/別リポジトリは未確認。
- 投稿APIの既存会話所有確認前の履歴参照、provided token hashでのcreateConversation（Firestore PATCH/メモリset）を確認。createConversationはcreated_atも毎回渡すため、既存会話の初回日時保持にも不備がある。実環境での攻撃試験はしていない。
- 新規会員管理画面が既存にあるかのような説明を撤回。既存は利用者設定/掲示板、Discord連絡、GA4分析。提案書冒頭へ未承認・実装仕様に使わない旨を追記。アプリソースの変更なし。

## 会員番号変更と購入区分の検討

- MEMBER_IDENTITY_DESIGN.mdへ、固定内部ID/表示番号/権利状態の分離案を保存。一般10000〜・有料1〜は維持。
- 推奨遷移: 一般番号→初購入時の有料番号、権利取消時は元一般番号、再購入時は既存有料番号。番号は再利用せず、会話IDと計測IDを維持。遷移規則は提案で未確定。
- 現Stripe Payment Linkを購入照合ID+署名検証Webhookで接続する最小案を検討。既存購入者の所有確認、Stripe本番設定、返金/複数端末方針は未確認/未確定。管理画面の新設は前提にしない。コード変更なし。

## 会員データの保存先案

- MEMBER_IDENTITY_DESIGN.mdへFirestore正本/PCキャッシュ/秘密保管/GA機能イベントの分離とコレクション案を追加。
- 既存settings.rsは設定全体を取得・全窓通知するため、秘密をSettingsへ載せない設計。会員台帳では既存のメモリフォールバックを禁止する案。
- ①固定ID、②一般10000〜、③有料1〜を紐付け、採番/会員作成をトランザクション化。データ配置は検討段階。本番設定・コード変更なし。

## 会員登録・購入自動採番の図中心設計書

- MEMBER_PAYMENT_SPEC.md v0.1を作成。Mermaid6図で①②③対応、保存先、初回登録、購入、自動化できない旧購入の引継ぎ、番号変更を説明。
- ユーザー操作とAPI/Stripe/Firestore/Rustの処理を区別し、既存画面への追加だけを提案。新規管理画面は対象外。
- 本番設定・所有確認・返金/複数端末は未確定として記載。設計作成のみ、アプリ/決済/配信変更なし。

## 会員番号先行導入の検討結果

- MEMBER_IDENTITY_DESIGN.mdへ第1段階の完成条件、API3件、作業A〜E、保存環境分離、公開条件を追記。
- 既存購入者を誤って未購入扱いしないため、③空欄と購入未確認を区別する。決済接続前は購入権利を変更しない。
- 第1段階は①②/既存会話対応/Discord番号表示まで。Stripe・有料採番・GA個人分析は後続。本番Firestoreと秘密保護は未検証であり、実装/公開完了とはしない。

## 会員番号と機能別利用分析を同一リリースへ拡張

- 最新ユーザー指定により、会員基盤だけでなく個別・全体の機能使用状況を確認できるところまで今回の範囲を拡張。上の「GA個人分析は後続」は今回の完成条件を表さない。
- MEMBER_USAGE_RELEASE_SCOPE.mdへ収集・表示・対象機能・修正範囲・実測レポートまでの合格条件を具体化。
- 日次最小集計をFirestoreへ送る案と開発者PCのHTML/CSVレポートを提案。現行docs-v2は匿名GA送信のみであるため、新経路/新同意/保持期間/再送の設計確認が必要。既存同意を流用しない。
- docs/010_RELEASE.mdを全文確認。STEP 0-1、設計具体化中。実装・配備・版変更は未着手。

## 2026-09-04 承認後の実装開始

- ユーザーOKで新収集・レポート方式を承認。codex/member-usageブランチを作成。docs-v2/009_MEMBERSHIP.mdに正式仕様とシーケンス2-1/3-1を追加。
- 会員Firestore専用保存・原子的採番・資格情報認証・同意世代・日次集計・開発者認証付き取得APIを実装中。APIはMEMBER_API_ENABLEDと環境設定が揃うまで無効。配備していない。
- 既存投稿APIの所有権確保を履歴取得/Discord送信より前へ移動。保存層で異なる秘密を拒否し初回日時/秘密ハッシュの上書きを防止。
- Claudeサブレビューを試みたがNot logged inで未実施。対象テスト/型検査を実行中。
- ユーザーから更新/アンインストール時の番号維持の質問。通常更新は維持、資格情報削除時は現案で新規番号、Store削除時の残存挙動は未検証と回答。正式仕様に通常更新/アンインストール/リセットを別々の確認項目として追加。
- 残作業: PC側保護保存/設定表示/新同意UI/機能成功箇所計測/既存会話接続/HTML-CSVレポート/正式仕様の既存記述整合/テストと実機/本番環境確認/リリース工程。現時点でリリース不可。

## 2026-09-05 会員分析の実装・検証状況

- PC側member_identity.rsをAppStateのserde(skip)領域に追加。LOCALAPPDATA/OreNoFusen/membership/{environment}/identity.binへWindows DPAPIで保護保存。バージョン非依存の保存先、壊れた資格情報の自動再発行禁止。Windows保護/復元とAppStateの秘密非公開テスト2件成功。
- 実行時MSIX名ONFStudios.FUSEN/ONFStudios.FUSEN.Devでproduction/developmentを分け、同じexeで開発MSIX確認後にStore梱包する手順と整合。最後の変更のcargo checkはビルドロック待ち。通常のStore更新・削除再インストールの実機試験はまだ。
- 会員API、同意世代、日次置換、90日削除Cron、Rustの最大7日待機集計、既存設定の番号/コピー/明示同意、会話接続とDiscord番号付記、HTML/CSVレポートを実装。接続/配備は未完了。
- 既存7機能＋作成、本文編集（編集終了時）、階層開閉（画面操作）、画像添付（ドラッグ）を接続。すべての同等操作経路を網羅したとは説明しない。saveNoteContentが実保存成功booleanを返し、スキップをfirst_note_saved/alarm_setの成功に数えない。複製の保存スキップも中止する。
- 対象検証: 会員/問い合わせ17件成功。拡張対象54件中52件成功、AnalyticsLoader2件は動的importの初期化をまとめて修正し単独3件成功。最新の会員/所有権/保存成否/複製/Loader22件成功。StickyNote19件、analytics16件成功。HTML/CSV集計2件成功。Rust cargo checkは途中版成功、DPAPI2件成功。TypeScriptは最後のLoader型修正後に再確認中。
- Vercelログイン済みChromeで、FIREBASE_PROJECT_ID/CLIENT_EMAIL/PRIVATE_KEYがProductionとPreviewに登録されていることを一覧で確認。秘密値は開かず、DB実接続/権限/TTLはまだ未検証。
- Vercel CLIは未導入。通常npx取得は無応答のため中断、offlineはENOTCACHED。昇格した `npx --yes vercel whoami` は自動承認レビューで拒否（未確認npmパッケージの取得実行と認証情報へのアクセスが具体的に未承認との理由）。回避せず、ユーザーへ公式CLIの接続/配備利用許可を非同期で質問済み。再許可前に別経路で同じCLIを実行しない。
- ClaudeはNot logged in。レビュー未実施。
- 現在: STEP 0-2。版変更、develop/main反映、MSIX、外部配備、Store提出は未実施。運用手順はdocs/MEMBER_USAGE_OPERATIONS.md。追加環境変数・検証配備・DB実照合・UI/更新/同意撤回/再インストール実機確認・公開が残る。
- 最終確認: package identityによる環境分離を含むcargo check --locked成功（47.45秒）、npx tsc --noEmit成功、git diff --check成功。上の型検査待ち記載を解消。Storeでの更新/再インストールの実機保証はまだ行わない。

## 2026-09-05 通信頻度の設計訂正

- ユーザー指摘により、利用日次集計をVercel/Firestoreへ送る案を撤回。会員番号登録は初回1回、通常起動ではPC保存値を使用する。
- 機能利用は既存GA4経路へ集約し、Firestoreには会員番号・固定ID・ランダム分析IDの対応だけを持つ。会員別利用履歴をVercel/Firestoreへ保存しない。
- ユーザー指定によりGA4もリアルタイム不要。月内の利用有無をPCで集計し、翌月の初回起動時に月次サマリーとして送る設計へ変更する。各操作でGA4通信しない。
- 先に実装した会員日次API/Rust同期/削除Cron/HTML-CSV日次レポートは撤去対象。設計書・同意文言・テストを月次GA4方式へ修正してから再検証する。現在リリース不可。

## 2026-09-05 本来機能優先の月次利用集計へ修正

- ユーザー承認により「すぐ書ける」を最優先とし、通常操作でディスク・Tauri IPC・GA4・Vercelへアクセスしない方式へ変更した。操作時は各ウィンドウのメモリ上の整数を増やすだけ。
- 60秒後に複数操作を1回のTauri IPCでRustへ渡し、Rustは月・固定機能名ごとに使用回数、使用日数、最終使用日をメモリ集計する。5分間隔で保護ローカル保存し、翌月以降の起動から60秒後にGA4へ月次送信する。
- GA4へは会員番号を送らず、Firestore会員台帳で一度だけ生成するランダム分析IDをUser-IDに使う。Firestore/Vercelには利用履歴を保存しない。会員番号APIは番号または分析IDがPCにない場合の登録時だけ呼ぶ。
- 旧日次usage/consent/report API、90日削除Cron、HTML/CSV日次レポートを撤去。会員登録APIは一般番号10000開始とランダム分析IDの再試行時保持に限定した。
- 操作100回中のTauri IPCが0回、60秒後に1回の一括通知になるテストを追加。Map加算100万回の実測は26.14ms、1回平均0.000026ms（環境内参考値）で10ms要件を十分下回った。
- 検証: 対象24件、全Vitest 110ファイル・560件、TypeScript、Rust cargo check、VitePress build、git diff --checkに成功。実機で起動/入力体感、開発会員番号発行、月跨ぎGA4受信を確認するまではリリース不可。GA4側には monthly_feature_usage 用のカスタム定義を追加する必要がある。

## 2026-09-05 利用分析を週次へ変更

- ユーザー判断により、改善判断を早くするため集計周期を月次から週次へ変更した。UTC月曜開始・日曜終了のISO週 `YYYY-Www` を使用し、翌週以降の起動から60秒後に前週分を送る。
- GA4イベントは `weekly_feature_usage` と `weekly_usage_complete`、期間パラメータは `summary_week` とする。回数・使用日数・最終使用日は維持する。
- ユーザー向けの利用回数・グラフ画面は追加しない。設定画面は会員番号と分析同意だけを表示し、「すぐ書ける」体験と画面の単純さを優先する。
- Vercelアクセス、操作時のメモリ加算、60秒一括IPC、5分間隔ローカル保存の負荷条件は変更しない。実機確認とGA4側の週次カスタム定義登録まではリリース不可。

## 2026-09-05 GA4週次カスタム定義登録

- GA4プロパティ `ore-no-fusen` へイベントスコープの「集計対象週」=`summary_week`、「最終使用日」=`last_used_day` を登録した。
- カスタム指標「週間使用回数」=`usage_count`、「週間使用日数」=`active_days` を標準単位で登録した。
- 既存 `feature_name` の説明を週次対象11機能へ更新し、`app_version` と `distribution` の表示名・説明を週次イベントに合わせた。既存パラメータ名は変更していない。
- 個別・全体の閲覧方法を `docs/MEMBER_USAGE_OPERATIONS.md` に記録。次は週次ペイロードの対象テスト、開発版の会員番号発行、週跨ぎ相当のGA4実通信確認。

## 2026-09-05 個別分析の会員番号照会

- 会員番号からGA4 User-IDを確認する運用がFirestore管理画面の手作業だけだったため、管理者専用 `POST /api/members/lookup` を追加。
- Vercelの `MEMBER_REPORT_TOKEN` によるBearer認証を必須とし、一般番号の索引から1件だけ直接検索する。返却は一般番号、匿名分析ID、登録日時、有料番号だけで、会員ID・秘密・利用履歴は返さない。
- 通常ユーザーの起動・付箋操作からは呼ばれず、アプリの通信回数と操作負荷は増えない。環境変数設定と開発Previewでの実接続は未実施。
- Vercelへ `MEMBER_ENVIRONMENT=development` をPreview限定で登録。`MEMBER_API_ENABLED=true` はProduction/Previewに登録されたが、Productionには `MEMBER_ENVIRONMENT` がないため本番APIは構成不成立のまま動作しない。開発Previewの次回デプロイから有効になる。
- 最終状態で全Vitest 110ファイル・562件、Rust会員テスト3件、TypeScript、Next.js本番ビルド、VitePressビルド、差分検査に成功。VitePressの誤ったルート実行1回はローカルパッケージを見つけられず失敗したが、正しい `npm --prefix docs-v2 run docs:build` で成功した。
...
## 2026-09-05 しまった付箋の選択復元（Phase 22 計画）

- タスクトレイ右クリックの「しまった付箋を取り出す」から、Archive・タグフォルダの付箋を検索・絞り込み・複数選択し、最後にまとめて復元する方針で合意。
- 復元はコピーではなく保存先直下への移動。成功後は元Markdownを削除し、関連画像も安全に戻す。
- 保存先直下に同名付箋がある項目は上書きせず、他項目の復元は継続して結果を表示する。
- `.planning/phases/22-/` に CONTEXT / RESEARCH / VALIDATION / 2つのPLANを作成。Wave 1でRustの安全な列挙・移動、Wave 2でトレイ・選択UI・即時表示・設計書・実機UATを行う。
- RESTORE-01〜05は2計画ですべて網羅。ソースコードは未変更。
- 2026-09-06 UI方針を追加決定。初期画面に最近しまった5件、その下に件数付きのArchive・タグ一覧を表示し、保存場所内は新しい順とする。
- 復元候補は横長の付箋カード1列。元の背景色、本文冒頭3行、最初の画像サムネイル、保存場所、しまった日付を表示し、カード全体またはチェックボックスで選択する。詳細表示後も選択を保持する。
- 今後はアーカイブ時に `archivedAt` を記録し、既存データはファイル更新日時へフォールバックする計画へ更新。復元時は `archivedAt` を除去する。

## 2026-09-06 文書のみのコミット前テスト省略

- `.husky/pre-commit` の先頭でステージ済みファイルを分類し、`.planning/`、`docs/`、`docs-v2/`、ルート直下MarkdownだけのコミットではTypeScript・全ユニット・E2Eを省略するよう変更。
- 文書以外が1件でも混ざる場合は、従来の検証をすべて維持する。
- Git for Windows同梱bashによる構文検査と、文書のみ・文書混在・フック変更の判定例に成功。全ユニットテストは目的に照らして実行していない。

## 2026-09-06 しまった付箋の選択復元（Phase 22 実装）

- `Archive` と直下の `tags/<タグ名>` を安全に列挙し、背景色・本文先頭3行・最初の画像・保存場所・`archivedAt` を返すRustコマンドを追加。旧データの日付は更新日時へフォールバックする。
- 復元は保存先直下へ移動し、同名本文は上書きしない。関連画像は衝突時に一意名へ変更し、他の付箋が共有している画像は元側へ残す。複数選択は成功・競合・失敗を個別に返す。
- タスクトレイから開く復元画面を追加。最近5件、保存場所別件数、新しい順、全文検索、背景色・3行・画像付きカード、詳細、選択保持、まとめて復元、即時表示に対応。
- 設計書 `002_PC.md` をv2.98へ更新し、ユーザーガイドへ操作手順を追加。
- 検証成功: 専用Vitest 6件、Rust専用7件、TypeScript、VitePress build、専用Playwright 1件、全Vitest 116ファイル・579件、全Rust 282件（2件ignore）、`git diff --check`。残りはWindows実機での操作確認。
- 実機指摘を反映。今後しまう付箋は `window`（位置・大きさ）を保持し、取り出し直後の自動タグ整列を廃止。既にしまってあり `window` がない旧データは元位置を復元できない。
- `fusen_archive_note` 成功時に `fusen:archive_changed` を通知し、開いたままの取り出し画面が一覧と保存場所別件数を即時再取得する。
...
- STEP 3-3完了: 2026-09-07、Partner CenterのSubmission 18へ`ore-no-fusen.msix`をアップロード。Version `5.2.2.0`、Architecture `X64`、Windows Desktop、Validatedを確認し、日本語・英語の更新情報を保存した。
- ユーザーの明示承認後にSubmission 18を送信し、「Update in certification」「Pre-processing」を確認。認定合格後すぐに公開する設定。次は公開後にStoreから更新し、版番号と起動を確認するSTEP 3-4。

## 2026-09-08 Codexコックピットの5時間・週間表示

- ユーザー確認後、別ツール `C:\Users\uck\usage-widget\codex-usage-widget-with-context.ps1` を修正。短期枠をWeeklyと表示・保存していた誤りを解消し、5時間と1週間の使用率・残率・リセット日時を2行表示。
- 取得ヘッダーの期間で分類。未知・欠落・旧形式は未取得。絶対リセット日時を保存し、両枠の残量低下を更新周期へ反映。
- PowerShell 5.1で構文、2枠、順序逆転、欠落・未知・範囲外、表示とバー、保存復元、旧形式、低残量を検証。フォーム描画を目視確認。実データ取得成功、本体バックアップ・反映・再起動済み。保存データに両枠が存在し取得エラーなし。ユーザー側で2行表示の確認待ち。
- 付箋アプリのコードは変更なし。

## 2026-09-08 Store 5.2.2 会員番号「登録待ち」の本番設定復旧

- ユーザー実機で初回起動1分超でも登録待ち。本番status APIがproductionに400 Wrong environmentを返すことを確認。
- 根本原因はVercel ProductionのMEMBER_ENVIRONMENT未設定。Preview限定の設定しかなく、過去の記録とも一致。ユーザーが本番設定修正を承認。
- VercelでProduction限定のConfig MEMBER_ENVIRONMENT=productionを保存。Current Productionのmain e67d84bを同一ソースで再配備開始。アプリ・版番号・MSIXは変更なし。
- 次: Ready、production/誤環境のAPI応答確認、ユーザーによるアプリ完全終了→再起動約1分後の会員番号表示確認。
- 復旧確認: 再配備 8mU68goyVKZ2MAMhFH3pKnSNE6up が2分31秒でReady、本番ドメインへの反映成功。読み取り専用statusに架空資格情報を送り、productionはFirestore照合後403 Invalid member credentials、developmentは400 Wrong environment。環境不一致は解消し、環境分離とDB読取りを確認。実会員の登録・番号表示はユーザーの再起動確認待ち。
- 実機確認完了: ユーザー提供のStore 5.2.2画面で「会員番号：10000」と接続先productionを確認。分析同意は未選択のまま番号発行成功。本番会員番号の登録待ち不具合は復旧完了。

## 2026-09-08 フィードバック表示文言の最小修正

- ユーザー依頼: Discordで会員番号ラベルが重複し、連絡先の説明が返信に必須と誤解される。
- 原因: Discordの項目名が「会員番号」で、共通値生成も「会員番号 10000」を返していた。共通値を数字だけへ変更。通常投稿と掲示板投稿の両方に適用。
- 日英の連絡先説明を、返信は「開発者とのやりとり」に表示され、連絡先入力は任意である旨へ変更。連絡先欄は維持。
- 対応仕様: 007 図6-1の開発者通知と掲示板表示、002 6.6の連絡先なしで会話可能という既存仕様、009の会話IDから会員番号を自動付与する既存仕様。データフロー変更なし。
- 未配布。既存Discord投稿はそのまま。今後の通知への適用にはサーバー配備、Store版の文言反映には次回アプリ更新が必要。
- 検証成功: 会員番号表示の回帰テスト3件、TypeScript型検査。指定したfeedback route.test.ts 2パスは既存ファイルがなく、実行対象は新規テスト1ファイルのみ。

## 2026-09-08 やりとりの画像添付・初期計画（未実装）

- ユーザー依頼は「まずどこに何が必要か」の計画。対象案はフィードバック初回送信、掲示板からのユーザー追送、Discordからの開発者返信の双方向。
- 現状: FeedbackConversationMessage/DiscordCandidateMessage/DiscordMessageに添付項目なし。Discord取り込みは本文のみで、空本文を拒否。画面はmessage.bodyだけを描画。007の添付画像は将来検討。
- 必要箇所: settings-pageの2送信画面と履歴表示、feedbackConversationの送受信型、feedback APIの添付受付/取得、types/storeの添付メタデータ、Discord Webhook添付送信とingestの画像取込、認証付き画像保存領域と削除処理。
- 保存案（未決定）: 本文・画像メタ情報をFirestore、画像実体を非公開ファイルストレージへ。本番/開発を分離。会話資格情報で画像アクセスを認証。製品・費用・既存契約は次段階で調査。
- 最小仕様案（未承認）: PNG/JPEG/WebP静止画、1投稿3枚・1枚5MBを仮上限。添付ボタンとクリップボード貼付、送信前プレビュー/取消、履歴サムネイル/拡大、画像だけの投稿。動画/PDF/画像編集は対象外。
- 検証計画: 双方向表示、画像のみ/本文のみ/混在、旧履歴互換、容量/形式拒否、別会話アクセス拒否、再試行重複防止、保存/Discord送信失敗、会話削除時の画像掃除、Discord側コピーの削除範囲を決定。
- 設計対象: 007 図6-1の送受信シーケンスを拡張、002の設定UI、100の明示添付画像の送信・保存・削除説明。配布はサーバー互換対応を先に行い、その後PC版。

### 画像添付の費用・上限案（26-09-08）
- ユーザー指定: 保存約1か月、保存期間の表示、1投稿3枚・1枚5MB、月間枚数制限可。安価・最低限を最優先。
- 推奨案: Cloudflare R2 Standardの非公開バケット（本番/開発分離）に画像、既存Firestoreにメタ情報。公式料金は10GB-month、書込系100万/月・読取系1000万/月が無料枠、外向き転送無料。アカウント既存使用量・有効化条件は未確認。Firebase StorageはBlaze要件があり、今回はR2を第一候補とする。
- 追加提案（未承認）: 会員ごと直近30日20枚（最大100MB）、全環境・開発者返信・未完了分含む添付保存5GBで新規受付停止。サーバー側で予約・計上し、同時送信でも超えない。通信回数制限も必要で無料保証ではない。
- 閲覧期限は送信後30日、期限後は画像だけ期限切れ表示。R2ライフサイクルで削除（通常期限から24時間程度の遅延があり、30日ぴったりの物理削除を約束しない）。Discord側コピーの削除は別途必要で、保存期間の説明範囲を確定する。
- 5MBはVercel Functionsの4.5MB本文制限を超えるため画像を既存JSON APIへ直接送らない。直接アップロード等の方式と実サイズ検査を詳細設計する。
- 参照: https://developers.cloudflare.com/r2/pricing/ 、https://developers.cloudflare.com/r2/buckets/object-lifecycles/ 、https://firebase.google.com/docs/storage/faqs-storage-changes-announced-sept-2024 、https://vercel.com/docs/functions/limitations
- 計画のみ。契約・設定・実装は未実施。

### 無課金必須への計画変更（26-09-08）
- ユーザー確定条件: 無課金が絶対。上限時は停止を許容し、自動従量課金は禁止。
- R2案は撤回。アプリの容量上限だけでは操作回数等の課金を防げない。
- 第一候補をAppwrite Cloud Freeへ変更（採用/契約/実装は未実施）。公式Free説明では追加リソース購入不可、容量上限でアップロード停止、帯域上限でAPIアクセス拒否。現行無料枠は組織共有2GB保存・5GB/月帯域、2プロジェクト。1週間非活動で休止する制約あり。
- 最小案: 画像用Free組織を有料化せず利用、本番/開発を分離。アプリ側容量は合計1GB程度へ縮小し、超過や保存先休止時は画像利用不可として扱う。従来の5GB上限・人数試算は適用しない。1投稿3枚/1枚5MB/保存約30日は維持。月20枚は提案段階。
- 全経路確認が実装開始条件: 画像アップロード/閲覧/削除/Discord取り込みと転送/メタデータ処理の実行先を確認し、既存Vercel・Firestore等への追加従量課金も起きない構成かを契約プラン含めて確認する。画像保存先Freeだけでシステム全体0円と断定しない。保証できない経路は実装しない。
- 停止中の期限切れ画像の物理削除とDiscordコピー削除は未解決。閲覧30日と順次削除を区別し詳細設計する。
- 公式確認: https://appwrite.io/docs/advanced/billing/free 、https://appwrite.io/pricing （2026-09-08参照）。

### 実現性評価用の全手順・担当・合格条件（26-09-08、最新計画）

計画のみ。従来の8手順を準備・試験・公開の12手順へ具体化。自動課金なし、A/B間で質問・返信・画像が漏れないことが必須。現時点の実現性は条件付きで、採用確定ではない。

| 手順 | 作業・成果物 | Codex | ユーザー | 合格条件 |
|---|---|---|---|---|
| 1 | 既存Vercel/Firestoreと候補保存先の契約・無料枠・超過動作の確認 | 読取確認、全経路の費用表作成 | 必要なサービスへのログイン | 画像機能の全処理で自動課金されない。保証不能なら中止/再設計 |
| 2 | 認証・画像転送・削除の最小方式を評価 | A/B分離、5MB転送、休止時の挙動を設計 | 保持・削除説明の確認 | 公開URL/転送可能な署名URLだけで保護しない。期限削除と無料停止の両立方法がある |
| 3 | 設計書007図6-1等と試験表を具体化 | 入出力・上限・月替わり・失敗処理・画面文言を記載 | 未確定の枚数等を確認 | 月20枚は暫定。サービス請求期間と直近30日を区別 |
| 4 | 無料アカウント準備 | 登録画面案内、Free確認 | アカウント登録、規約同意、本人認証 | 有料化しない。既存アカウントの有料組織と混同しない |
| 5 | 隔離検証環境準備 | A/Bの別資格情報・別プロファイル、架空質問・画像、検証保存先を準備 | 本人操作が必要な場合のみ対応 | 本番台帳・本物のユーザーデータを使用しない |
| 6 | 検証Discord準備と操作可否確認 | 検証チャンネルとBot/Webhook/許可開発者/検証APIの対応確認 | チャンネル指定、テスト投稿・返信の許可、必要な権限操作 | 本番への誤投稿・取り込みなし。ブラウザ操作できない部分はユーザー手動で同じ試験 |
| 7 | 最小の実接続試作と実現性判定 | Aの画像送信、Discordの返信画像取込、Aの閲覧とBの拒否、無料経路を確認 | 必要時Discord操作 | ここまでで採用可否を報告。失敗なら本実装へ進まない |
| 8 | 本実装 | 添付/取消/プレビュー/拡大、認証付き取得、予約計上、停止/再開、削除/再試行、日英表示 | 原則なし | 文章経路と旧版互換を保つ。使用量不明時は画像を停止 |
| 9 | 自動試験・API直接試験 | 正常/境界/異常/同時実行とA/B逆方向アクセスを実行 | なし | 下記試験群に合格 |
| 10 | Discordを含む結合試験とWindows実機確認 | A/Bを交互に操作、Discord返信、ログ/結果記録。ネイティブ操作可否は事前確認 | 貼付/選択/拡大等、Codexで操作できない実機項目 | 実際の送受信と誤配信なしを確認。画面非表示だけで合格にしない |
| 11 | 試験結果評価・テストデータ片付け | 残存画像/投稿/資格情報と使用量を確認、必要な削除を準備 | 不可逆削除の確認が必要な場合のみ操作/承認 | 未解決事項と削除範囲を記録。無料停止中の削除可否も明記 |
| 12 | リリース・本番確認 | docs/010_RELEASE.mdのSTEP 0〜3に従い互換サーバー/PC配布を準備 | 公開承認、Store実機確認 | 本番の設定不足とA/B分離を再確認。新たな大容量負荷試験はしない |

必須試験群:
- A/B分離: A資格情報+B会話ID/質問ID/返信ID/画像IDの全取得/変更/削除を拒否し、B→Aも同様。無認証・不正認証・列挙要求も拒否。URLコピーだけで画像が見えない。公開バケット/CDNキャッシュから迂回取得不可。秘密をURL/ログへ出さない。UI・API・保存先の実接続で実施。
- Discord: A/Bそれぞれの投稿へ別画像で返信し、正しい相手だけへ届く。許可外の投稿者、無関係な投稿への返信、重複取込を拒否。本文なし画像返信も確認。Discordチャンネルは開発者用で、一般ユーザーに開放しない。
- 基本: 本文のみ/画像のみ/混在、3枚成功/4枚拒否、5MBちょうど/超過、許可形式/偽装/破損/過大画素数、選択/貼付/取消/再送。
- 上限: ユーザー別枚数・全体容量・帯域の閾値前後、同時送信予約、未完了/開発者添付の計上、上限前の月内停止、文章継続、閲覧停止。測定不能なら停止。
- 再開/保持: 請求期間回復+空き容量の両方確認、未回復時は継続停止。仮時計で30日期限、実削除APIで削除確認、停止/休止時の失敗と復旧後掃除。Discordコピーの扱いも確認。
- 障害/互換: アップロード途中切断、DB/Discord失敗、繰返し再送、予約解放、孤立画像、キャッシュ利用者混同、旧版文章投稿/既読/会員番号維持。
- 費用は実際の無料枠を使い切らず、小さな検証閾値で停止動作を確認。契約上の非課金とアプリ上限試験を区別する。

主要未解決: 無料保存先の認証付き画像配信を既存会話資格情報とどう接続するか、5MBを既存API制限内で転送する方法、休止/帯域停止中の削除、Discordコピー削除、既存有料経路の遮断。Discordブラウザ操作・Bot権限・A/B Tauri実機隔離は実行前に確認し、現時点で全自動可能とは断定しない。

## 2026-09-08 会員番号重複修正の検証配備
- ユーザー承認で修正3ファイルのみコミットe0cdcf7cdab90f67e908bcf29a93765c35efe4c3、developへpush。本番main未変更。
- pre-commit: 全118ファイル586テスト・TypeScript成功。Vercel Preview配備FSRBxASTFAa4MmaYN9hrVcCGNV4Y成功。develop固定URLのmember statusへdevelopmentの無資格情報要求が403となり、開発用環境の受付を確認。
- GitHub Test run 34221091828: frontend成功、Rust cargo testは記録時点で継続中。CI全体成功とは未判定。
- 次: ユーザーが開発版から新規投稿し、Discordで項目名「会員番号」の下が数字だけになることを確認。既存Discord投稿は変更されない。こちらからDiscordへのテスト投稿は行っていない。
- 実機確認成功: ユーザーの開発版からの新規投稿で、Discord項目名「会員番号」の値が10001のみとなり重複解消を確認。ユーザーよりコミット依頼。修正コードはe0cdcf7でコミット・develop配備済み。本番反映は未実施。
