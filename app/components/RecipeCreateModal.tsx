...
export default function RecipeCreateModal({
    ...
    const confirmOverwriteDraft = useCallback(() => {
        ...
        return window.confirm(isEnglish
            ? 'Rebuild the manually edited draft from the selected materials?'
            : '手編集した叩き台を材料から作り直します。よろしいですか？');
    }, [isDraftEdited, isEnglish]);

    useEffect(() => {
        let cancelled = false;
        loadLocalizedCrystalFormats(settings.language).then((formats) => {
            if (!cancelled) {
                setRecipeFormat(formats.recipe);
            }
        });
    ...
    useEffect(() => {
        ...
        getRecipeCandidates(sourcePath)
            ...
            .catch((e) => {
                ...
                setError(isEnglish ? 'Failed to load material candidates' : '材料候補の取得に失敗しました');
            })
            .finally(() => {
                if (!cancelled) setIsLoading(false);
            });
        return () => { cancelled = true; };
    }, [isEnglish, sourcePath]);
    ...
    useEffect(() => {
        ...
        regenerate().catch((e) => {
            ...
            setError(isEnglish ? 'Failed to build the draft' : '叩き台の生成に失敗しました');
        });

        return () => { cancelled = true; };
    }, [isEnglish, recipeFormat, selectedYellowPath, sourceBody]);
    ...
    const handleCreate = useCallback(async () => {
        ...
        try {
            ...
            setError(isEnglish ? 'Failed to create the recipe' : 'レシピの作成に失敗しました');
        } finally {
            setIsCreating(false);
        }
    }, [draftBody, fallbackTitle, isCreating, isEnglish, onClose, onCreated, recipeFormat, sourceTags]);
    ...
    return (
        <CrystalCreateModalShell
            ...
            createLabel={isEnglish ? 'Create Recipe' : 'レシピを作る'}
            isCreating={isCreating}
            accent="orange"
            language={settings.language}
        >
                <section className="flex max-h-[36%] shrink-0 flex-col gap-1 overflow-y-auto pr-1">
                    <h4 className="pr-8 text-xs font-bold text-gray-600">{isEnglish ? 'Choose materials' : '材料を選ぶ'}</h4>
                    {isLoading && <div className="text-sm text-gray-500">{isEnglish ? 'Looking for materials...' : '材料を探しています...'}</div>}
                    {noCandidates && (
                        <div className="text-xs text-gray-600 bg-gray-50 border border-gray-200 rounded-lg p-2">
                            {isEnglish ? 'No materials have the same tag' : '同じタグの材料はありません'}
                        </div>
                    )}

                    {candidates.yellows.length > 0 && (
                        <div className="flex flex-col gap-2">
                            <div className="text-xs font-bold text-gray-500">{isEnglish ? 'Yellow (0–1) — added to Source' : '黄色（0〜1件）— 選ぶと「きっかけ」に入ります'}</div>
                            ...
                            >
                                {isEnglish ? 'Clear yellow selection' : '黄色の選択を外す'}
                            </button>
                        </div>
                    )}
                </section>
                ...
                <label className="flex min-h-0 flex-1 flex-col gap-1">
                    <span className="text-xs text-gray-500">{isEnglish ? 'You can create it as-is. Delete only the lines you do not need.' : 'そのまま作れます。不要な行だけ削除してください'}</span>
                    {error && <div className="text-sm text-red-600">{error}</div>}
                    <textarea
                        autoFocus
                        aria-label={isEnglish ? 'Recipe draft' : '手順の叩き台'}
